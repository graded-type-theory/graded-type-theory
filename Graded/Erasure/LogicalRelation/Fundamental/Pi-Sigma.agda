...
opaque
  ...
  lamʳ :
    ...
    Γ ∙ A ⊢ t ∷ B →
    γ ∙ ⌜ m ⌝ · p ▸ Γ ∙ A ⊩ʳ t ∷[ m ] B →
    γ ▸ Γ ⊩ʳ lam p t ∷[ m ] Π p , q ▷ A ▹ B
  lamʳ {m = 𝟘ᵐ} _ _ _ =
    ▸⊩ʳ∷[𝟘ᵐ]
  lamʳ {p} {Γ} {A} {t} {B} {γ} {m = 𝟙ᵐ} ok ⊢t ⊩ʳt =
    ...
    ®∷Π⇔ .proj₂
      ...
      , λ t′ ⊢t′ →
          ...
          case ⊩ˢ∷∙⇔′ .proj₂ ((_ , ⊩A) , (_ , ⊩t′) , ⊩σ) of λ
            ⊩σ,t′ →
          case redMany $
               β-red-⇒ (subst-⊢∷-⇑ ⊢t (escape-⊩ˢ∷ ⊩σ .proj₂)) ⊢t′
                 ok of λ
            lam-t[σ]∘t′⇒* →
...
opaque
  ...
  ∘ʳ :
    ...
    γ ▸ Γ ⊩ʳ t ∷[ m ] Π p , q ▷ A ▹ B →
    δ ▸ Γ ⊩ʳ u ∷[ m ᵐ· p ] A →
    γ +ᶜ p ·ᶜ δ ▸ Γ ⊩ʳ t ∘⟨ p ⟩ u ∷[ m ] B [ u ]₀
  ∘ʳ {m = 𝟘ᵐ} _ _ _ =
    ▸⊩ʳ∷[𝟘ᵐ]
  ∘ʳ {Γ} {u} {A} {γ} {t} {m = 𝟙ᵐ} {p} {q} {B} {δ} ⊢u ⊩ʳt ⊩ʳu =
    ▸⊩ʳ∷⇔ .proj₂ λ {σ = σ} {σ′ = σ′} ⊩σ σ®σ′ →
    case
      (t ∘⟨ 𝟘 ⟩ u) [ σ ] ®
    ...
    (t [ σ ] ® erase str t T.[ σ′ ] ∷ (Π p , q ▷ A ▹ B) [ σ ])  →⟨ (λ hyp →
                                                                      hyp _ $ R.escape-⊩∷ $ ⊩ᵛ∷→⊩ˢ∷→⊩[]∷ (fundamental-⊩ᵛ∷ ⊢u .proj₂) ⊩σ) ∘→
                                                                   proj₂ ∘→ proj₂ ∘→ ®∷Π⇔ .proj₁ ⟩
    (p PE.≡ 𝟘 →
...
opaque
  ...
  prodʳ :
    ...
    γ ▸ Γ ⊩ʳ t ∷[ m ᵐ· p ] A →
    δ ▸ Γ ⊩ʳ u ∷[ m ] B [ t ]₀ →
    ((p ·ᶜ γ) ⊕ᶜ δ) ▸ Γ ⊩ʳ prod s p t u ∷[ m ] Σ⟨ s ⟩ p , q ▷ A ▹ B
  prodʳ {m = 𝟘ᵐ} _ _ _ _ _ _ _ _ =
    ▸⊩ʳ∷[𝟘ᵐ]
  prodʳ
    {s} {p} {Γ} {A} {B} {t} {u} {γ} {m = 𝟙ᵐ} {δ}
    ...
    case
      (λ p≢𝟘 →
         ...
         σ ® σ′ ∷[ 𝟙ᵐ ᵐ· p ] Γ ◂ γ                 →⟨ ®∷→®∷◂ω (non-trivial ∘→ PE.trans (PE.cong ⌜_⌝ 𝟙ᵐ≡⌞p⌟)) ∘→
                                                      ▸⊩ʳ∷⇔ .proj₁ ⊩ʳt ⊩σ ⟩
...
opaque
  ...
  prodˢʳ :
    ...
    γ ▸ Γ ⊩ʳ t ∷[ m ᵐ· p ] A →
    δ ▸ Γ ⊩ʳ u ∷[ m ] B [ t ]₀ →
    p ·ᶜ γ ∧ᶜ δ ▸ Γ ⊩ʳ prodˢ p t u ∷[ m ] Σˢ p , q ▷ A ▹ B
...
opaque
  ...
  prodʷʳ :
    ...
    γ ▸ Γ ⊩ʳ t ∷[ m ᵐ· p ] A →
    δ ▸ Γ ⊩ʳ u ∷[ m ] B [ t ]₀ →
    p ·ᶜ γ +ᶜ δ ▸ Γ ⊩ʳ prodʷ p t u ∷[ m ] Σʷ p , q ▷ A ▹ B
...
opaque
  ...
  sndʳ {Γ} {t} {p} {q} {A} {B} {γ} {m = 𝟙ᵐ} ⊢t ⊩ʳt =
    ...
    case inversion-ΠΣ (wf-⊢∷ ⊢t) of λ
      (_ , ⊢B , ok) →
    case subst-⊢-⇑ ⊢B (escape-⊩ˢ∷ ⊩σ .proj₂) of λ
      ⊢B[σ⇑] →
...
opaque
  ...
  prodrecʳ :
    ...
    γ ▸ Γ ⊩ʳ t ∷[ m ᵐ· r ] Σʷ p , q ▷ A ▹ B →
    δ ∙ ⌜ m ⌝ · r · p ∙ ⌜ m ⌝ · r ▸ Γ ∙ A ∙ B ⊩ʳ u ∷[ m ]
      C [ prodʷ p (var x1) (var x0) ]↑² →
    (r PE.≡ 𝟘 → Empty-con Δ) →
    r ·ᶜ γ +ᶜ δ ▸ Γ ⊩ʳ prodrec r p q′ C t u ∷[ m ] C [ t ]₀
  prodrecʳ {m = 𝟘ᵐ} _ _ _ _ _ _ =
    ▸⊩ʳ∷[𝟘ᵐ]
  prodrecʳ
    {n} {p} {q} {A} {B} {C} {t} {u} {γ} {m = 𝟙ᵐ} {r} {δ}
    ...
    σ ® σ′ ∷[ 𝟙ᵐ ] Γ ◂ δ                                       →⟨ (λ σ®σ′ → ®∷[]∙◂∙⇔ .proj₂ (t₂®v₂′ , ®∷[]∙◂∙⇔ .proj₂ (t₁®v₁′ , σ®σ′))) ⟩
    ...
    module Lemmas
      ...
      private opaque
        ...
        r≢𝟘-lemma r≢𝟘 =                                              $⟨ σ®σ′ ⟩
          ...
          σ ® σ′ ∷[ 𝟙ᵐ ] Γ ◂ γ                                       →⟨ ®∷→®∷◂ω non-trivial ∘→
                                                                        ▸⊩ʳ∷⇔ .proj₁
                                                                          (PE.subst₂ (_▸_⊩ʳ_∷[_]_ _ _ _) (≢𝟘→ᵐ·≡ r≢𝟘) PE.refl ⊩ʳt) ⊩σ ⟩
      ...
      private opaque mutual
        ...
        ⊩C : Γ ∙ Σʷ p , q ▷ A ▹ B ⊩ᵛ⟨ l′ ⟩ C
        ⊩C = fundamental-⊩ᵛ ⊢C .proj₂
