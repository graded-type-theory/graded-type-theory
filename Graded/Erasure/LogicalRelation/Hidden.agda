  ...
  ®∷Π⇔ {p} {B} =
      (λ (_ , ⊩Π , t®v) →
         ...
         , λ t′ ⊢t′ →
             ...
             case reducible-⊩∷ $
               ...
               (_ , ⊩A , ⊩t′) →
             case IR.irrelevanceTerm ⊩A (_⊩ₗB⟨_⟩_.[F] ⊩Π′ id (id ⊢Δ))
                    ⊩t′ of λ
               ⊩t′ →
             case PE.subst (_⊩⟨_⟩_ _ _)
                    (PE.cong _[ _ ]₀ $ wk-lift-id B) $
                  _⊩ₗB⟨_⟩_.[G] ⊩Π′ id (id ⊢Δ) ⊩t′ of λ
               ⊩B[t′] →
               (λ { PE.refl →
                    _ , ⊩B[t′]
                  , irrelevanceTerm′ (PE.cong _[ t′ ]₀ $ wk-lift-id B)
                      (_⊩ₗB⟨_⟩_.[G] ⊩Π′ id (id ⊢Δ) ⊩t′) ⊩B[t′]
    ...
    lemma {⊩A} {⊩B} (yes PE.refl) t®v ⊩t′ =
      case PE.subst (_⊩⟨_⟩_ _ _) (wk-id _) ⊩A of λ
        ⊩A′ →
      case t®v _
             (PE.subst (_⊢_∷_ _ _) (wk-id _) $ escape-⊩∷ (⊩A , ⊩t′))
             .proj₁ PE.refl of λ
        (_ , ⊩B′ , tt′®v) →
...
opaque
  ...
  ®∷Σ⇔ :
    ...
    (ts » Δ ⊢ Σ⟨ s ⟩ p , q ▷ A ▹ B ×
     ∃₃ λ t₁ t₂ v₂ →
     ts » Δ ⊢ t ⇒* prod s p t₁ t₂ ∷ Σ⟨ s ⟩ p , q ▷ A ▹ B ×
     t₂ ® v₂ ∷ B [ t₁ ]₀ ×
     (p PE.≡ 𝟘 → vs T.⊢ v ⇒* v₂) ×
  ...
  ®∷Σ⇔ {t} {v} {s} {p} {q} {A} {B} =
      (λ (_ , ⊩Σ , t®v) →
         ...
         case B-PE-injectivity (BΣ _ _ _) (BΣ _ _ _)
           ...
           (PE.refl , PE.refl , _) →
         let ⊩wk-A     = _⊩ₗB⟨_⟩_.[F] ⊩Σ′ id (id ⊢Δ)
             ⊩wk-B[t₁] = _⊩ₗB⟨_⟩_.[G] ⊩Σ′ id (id ⊢Δ) ⊩t₁
         in
         case PE.subst (_⊩⟨_⟩_ _ _) (wk-id _) ⊩wk-A of λ
           ⊩A →
         case PE.subst (_⊩⟨_⟩_ _ _)
                (PE.cong _[ t₁ ]₀ $ wk-lift-id B) ⊩wk-B[t₁] of λ
           ⊩B[t₁] →
         (ts » Δ ⊢ Σ⟨ s ⟩ p , q ▷ A ▹ B ×
          ∃₃ λ t₁ t₂ v₂ →
          ts » Δ ⊢ t ⇒* prod s p t₁ t₂ ∷ Σ⟨ s ⟩ p , q ▷ A ▹ B ×
    ...
    , (λ (⊢Σ , _ , _ , v₂ , t⇒*prod , (_ , ⊩B , t₂®v₂) , hyp₁ , hyp₂) →
         case ⊩ΠΣ⇔ .proj₁ (reducible-⊩ ⊢Σ .proj₂) of λ
           ⊩Σ′@(_ , rest) →
         let ⊩wk-A , wk-B≡wk-B = rest id (id ⊢Δ) in
...
opaque
  ...
  ®∷Σ₀⇔ :
    ...
    (ts » Δ ⊢ Σ⟨ s ⟩ 𝟘 , q ▷ A ▹ B ×
     ∃₃ λ t₁ t₂ v′ →
     ts » Δ ⊢ t ⇒* prod s 𝟘 t₁ t₂ ∷ Σ⟨ s ⟩ 𝟘 , q ▷ A ▹ B ×
     vs T.⊢ v ⇒* v′ ×
     t₂ ® v′ ∷ B [ t₁ ]₀)
  ®∷Σ₀⇔ {t} {v} {s} {q} {A} {B} =
    t ® v ∷ Σ⟨ s ⟩ 𝟘 , q ▷ A ▹ B                                 ⇔⟨ ®∷Σ⇔ ⟩
...
opaque
  ...
  ▸⊩ʳ∷[∣]→®∷◂ {m} {n} {t} {A} defs-ok =
    ...
    (∀ {σ σ′} → ts » Δ ⊩ˢ σ ∷ Δ → σ ® σ′ ∷[ m ∣ n ] Δ ◂ 𝟘ᶜ →
     t [ σ ] ® erase str t T.[ σ′ ] ∷ A [ σ ] ◂ ⌜ m ⌝)                   →⟨ (λ hyp → hyp (⊩ˢ∷-idSubst (valid ⊢Δ)) (®∷[∣]◂𝟘ᶜ defs-ok)) ⟩
...
opaque
  ...
  conv-▸⊩ʳ∷ :
    ...
    γ ▸ Γ ⊩ʳ t ∷[ m ∣ n ] A →
    γ ▸ Γ ⊩ʳ t ∷[ m ∣ n ] B
  conv-▸⊩ʳ∷ {Γ} {l} {A} {B} {γ} {t} {m} {n} A≡B =
    γ ▸ Γ ⊩ʳ t ∷[ m ∣ n ] A                                  ⇔⟨ ▸⊩ʳ∷⇔ ⟩→

    (∀ {σ σ′} → ts » Δ ⊩ˢ σ ∷ Γ → σ ® σ′ ∷[ m ∣ n ] Γ ◂ γ →
     t [ σ ] ® erase str t T.[ σ′ ] ∷ A [ σ ] ◂ ⌜ m ⌝)       →⟨ (λ hyp ⊩σ σ®σ′ →
                                                                   conv-®∷◂ (⊩ᵛ≡⇔′ʰ .proj₁ A≡B .proj₂ .proj₂ id ⊩σ) $
                                                                   hyp ⊩σ σ®σ′) ⟩
    (∀ {σ σ′} → ts » Δ ⊩ˢ σ ∷ Γ → σ ® σ′ ∷[ m ∣ n ] Γ ◂ γ →
