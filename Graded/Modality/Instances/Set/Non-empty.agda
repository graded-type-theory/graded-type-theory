...
-- that is missing is distributivity of multiplication over addition.
-- This property holds if equality is replaced by inequality in a
-- certain way, but if the definition of a modality is changed so that
-- only the weaker variant of distributivity from this module is
-- required, then it seems hard to prove the substitution lemma in
-- Graded.Substitution.Properties.
