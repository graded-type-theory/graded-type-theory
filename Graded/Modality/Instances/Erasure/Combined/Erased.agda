...
module _ (ok-𝟘ᵐ : Allowed-at-𝟘ᵐ) where
  ...
  opaque
    ...
    ⊢∷-erased ⊢t =
      let ⊢t′ = ⊢∷[]→⊢∷ ⊢t
          ⊢A  = ⊢←⊢ ok-𝟘ᵐ (inversion-Erased (wf-⊢∷ ⊢t′) .proj₁)
      in
      ⊢∷[]←⊢∷▸ ok-𝟘ᵐ (erasedⱼ ⊢t′)
        (▸-cong (PE.sym ⌞𝟘⌟′) $
         ▸erased _ (▸-cong ⌞𝟘⌟′ (⊢∷[]→▸ ⊢t))
  ...
  opaque
    ...
    ⊢∷-mapᴱ {s} ⊢t ⊢u =
      let ⊢u′ = ⊢∷[]→⊢∷ ⊢u
          ⊢A  = ⊢←⊢ ok-𝟘ᵐ (inversion-Erased (wf-⊢∷ ⊢u′) .proj₁)
      in
      ⊢∷[]←⊢∷▸ ok-𝟘ᵐ (⊢mapᴱ (⊢∷[]→⊢∷ ⊢t) ⊢u′)
        (▸mapᴱ s (λ _ → Allowed-at-𝟘ᵐ.pr ok-𝟘ᵐ _ _ _)
            (λ _ → _ , ▸-cong ⌞𝟘⌟′ (⊢[]→▸ ⊢A))
           (▸-cong ⌞𝟘⌟′ (⊢∷[]→▸ ⊢t)) (▸-cong ⌞𝟘⌟′ (⊢∷[]→▸ ⊢u)))
  ...
  opaque
    ...
    ⊢≡∷-mapᴱ-[] ok ⊢t ⊢u = ⊢≡∷←⊢≡∷ ok-𝟘ᵐ $ mapᴱ-β ok (⊢∷[]→⊢∷ ⊢t) (⊢∷[]→⊢∷ ⊢u)
