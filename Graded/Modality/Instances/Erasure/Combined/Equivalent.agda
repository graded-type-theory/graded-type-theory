...
opaque mutual
  ...
  … | no not-erased =
    sub
      (Idₘ not-erased (⊢→▸? ⊢A)
         (▸-cong (PE.cong ⌞_⌟ (hyp not-erased .proj₂)) (⊢∷[]→▸ ⊢t))
         (▸-cong (PE.cong ⌞_⌟ (hyp not-erased .proj₂)) (⊢∷[]→▸ ⊢u)))
      (begin
         γ       ≡˘⟨ hyp not-erased .proj₁ ⟩
         δ       ≡˘⟨ +ᶜ-idem _ ⟩
         δ +ᶜ δ  ∎)
    where
    open ≤ᶜ-reasoning
  ...
  ⊢∷[]→▸ (snd ⊢t) =
    sndₘ (⊢∷[]→▸ ⊢t)
  ⊢∷[]→▸ {γ} {p} (prodrec {r} {p = p′} ok ⊢C ⊢t ⊢u) =
    sub
      (prodrecₘ (▸-cong (PE.sym ⌞⌟ᵐ·) (⊢∷[]→▸ ⊢t))
         (sub (⊢∷[]→▸ ⊢u) $ (begin
            γ ∙ ⌜ ⌞ p ⌟ ⌝ · (r · p′) ∙ ⌜ ⌞ p ⌟ ⌝ · r  ≈⟨ ≈ᶜ-refl ∙ M.·-congʳ (⌜⌞⌟⌝ _) ∙ M.·-congʳ (⌜⌞⌟⌝ _) ⟩
            γ ∙ p · (r · p′) ∙ p · r                  ∎))
  ...
  ⊢∷[]→▸ (suc ⊢t) =
    sucₘ (⊢∷[]→▸ ⊢t)
  ⊢∷[]→▸ {γ} {p} (natrec {p = p′} {r} ⊢A ⊢t ⊢u ⊢v) =
    let ▸A = sub (⊢→▸? ⊢A) (greatest-elemᶜ (𝟘ᶜ ∙ _))
        ▸t = ⊢∷[]→▸ ⊢t
        ▸u =
          sub (⊢∷[]→▸ ⊢u) $ begin
            γ ∙ ⌜ ⌞ p ⌟ ⌝ · p′ ∙ ⌜ ⌞ p ⌟ ⌝ · r  ≈⟨ ≈ᶜ-refl ∙ M.·-congʳ (⌜⌞⌟⌝ _) ∙ M.·-congʳ (⌜⌞⌟⌝ _) ⟩
            γ ∙ p · p′ ∙ p · r                  ∎
        ▸v = ⊢∷[]→▸ ⊢v
    in
...
module _
  ...
  private module Variants (hyp : ∀ {s₁} → s₁ <ˢ s₂ → P s₁) where
    ...
    opaque
      ...
      -- Variant of some of the variants.
    ...
    ⊢∷[]←⊢∷▸ {γ} {p = o} = λ where
      ...
      (prodrecⱼ {p} {r} {q} ⊢C ⊢t ⊢u _) ▸pr →
        ...
        prodrec ok (⊢←⊢ ⊢C)
          ...
          (⊢∷[]←⊢∷▸ ⊢u $
           sub ▸u $ (begin
             γ           ∙ o         · (r · p) ∙ o         · r  ≤⟨ γ≤rδ+η ∙ ≤-refl ∙ ≤-refl ⟩
             r ·ᶜ δ +ᶜ η ∙ o         · (r · p) ∙ o         · r  ≤⟨ +ᶜ-decreasingʳ _ _ ∙ ·≤⌜⌞⌟⌝· ∙ ·≤⌜⌞⌟⌝· ⟩
             η           ∙ ⌜ ⌞ o ⌟ ⌝ · (r · p) ∙ ⌜ ⌞ o ⌟ ⌝ · r  ∎))
      (ℕⱼ ⊢Γ) _ →
        ℕ (⊢←⊢′ ⊢Γ)
      (zeroⱼ ⊢Γ) _ →
        ...
        case more of λ where
            ...
            natrec ⊢A
              ...
              (⊢∷[]←⊢∷▸ ⊢u $
               sub ▸u $ let open ≤ᶜ-reasoning in begin
                 γ           ∙ o         · p ∙ o         · r  ≤⟨ γ≤δ+η+θ ∙ ≤-refl ∙ ≤-refl ⟩
                 δ +ᶜ η +ᶜ θ ∙ o         · p ∙ o         · r  ≤⟨ +ᶜ-decreasingʳ _ _ ∙ ≤-refl ∙ ≤-refl ⟩
                 η +ᶜ θ      ∙ o         · p ∙ o         · r  ≤⟨ +ᶜ-decreasingˡ _ _ ∙ ·≤⌜⌞⌟⌝· ∙ ·≤⌜⌞⌟⌝· ⟩
                 η           ∙ ⌜ ⌞ o ⌟ ⌝ · p ∙ ⌜ ⌞ o ⌟ ⌝ · r  ∎)
              (⊢∷[]←⊢∷▸ ⊢v $
          ...
          (invUsageNatrecNoNrGLB {χ} {x = q} glb₁ glb₂) →
            ...
            natrec ⊢A
              ...
              (⊢∷[]←⊢∷▸ ⊢u $
               sub ▸u $ begin
                 γ      ∙ o         · p ∙ o         · r  ≤⟨ γ≤δ+η ∙ ≤-refl ∙ ≤-refl ⟩
                 δ +ᶜ η ∙ o         · p ∙ o         · r  ≤⟨ +ᶜ-decreasingʳ _ _ ∙ ·≤⌜⌞⌟⌝· ∙ ·≤⌜⌞⌟⌝· ⟩
                 η      ∙ ⌜ ⌞ o ⌟ ⌝ · p ∙ ⌜ ⌞ o ⌟ ⌝ · r  ∎)
              (⊢∷[]←⊢∷▸ ⊢v $
               sub ▸v $ begin
                 γ       ≤⟨ γ≤θ+χ ⟩
        ...
        case inv-usage-J ▸J of λ where
          (invUsageJ
            ...
            J (λ _ _ →
              ...
              (⊢[]←⊢▸ ⊢B $
               sub ▸B $ begin
                 γ                    ∙ o         · p ∙ o         · q  ≤⟨ γ≤′ ∙ ≤-refl ∙ ≤-refl ⟩
                 γ₃ +ᶜ γ₄ +ᶜ γ₅ +ᶜ γ₆ ∙ o         · p ∙ o         · q  ≤⟨ +ᶜ-decreasingˡ _ _ ∙ ·≤⌜⌞⌟⌝· ∙ ·≤⌜⌞⌟⌝· ⟩
                 γ₃                   ∙ ⌜ ⌞ o ⌟ ⌝ · p ∙ ⌜ ⌞ o ⌟ ⌝ · q  ∎)
              (⊢∷[]←⊢∷▸ ⊢u $
               sub ▸u $ begin
                 γ               ≤⟨ γ≤″ ⟩
          ...
          (invUsageK {γ₂} {γ₃} {γ₄} {γ₅} ≤some p≢𝟘 _ ▸t ▸B ▸u ▸v γ≤) →
            ...
            K (λ _ _ → PE.refl , PE.refl , PE.refl , PE.refl , PE.refl)
              ...
              (⊢[]←⊢▸ ⊢B $
               sub ▸B $ begin
                 γ              ∙ o         · p  ≤⟨ γ≤′ ∙ ≤-refl ⟩
                 γ₃ +ᶜ γ₄ +ᶜ γ₅ ∙ o         · p  ≤⟨ +ᶜ-decreasingˡ _ _ ∙ ·≤⌜⌞⌟⌝· ⟩
                 γ₃             ∙ ⌜ ⌞ o ⌟ ⌝ · p  ∎)
              (⊢∷[]←⊢∷▸ ⊢u $
               sub ▸u $ begin
                 γ         ≤⟨ γ≤″ ⟩
