...
opaque
  ...
  -- Not having sucₑ on the stack is preserved by reduction

  suc∉-⇒ₑ : suc∉ S → ⟨ H , t , ρ , S ⟩ ⇒ₑ ⟨ H′ , t′ , ρ′ , S′ ⟩ → suc∉ S′
  suc∉-⇒ₑ suc∉ appₕ = (λ ()) ∙ suc∉
  suc∉-⇒ₑ suc∉ fstₕ = (λ ()) ∙ suc∉
  suc∉-⇒ₑ suc∉ sndₕ = (λ ()) ∙ suc∉
  suc∉-⇒ₑ suc∉ prodrecₕ = (λ ()) ∙ suc∉
  suc∉-⇒ₑ suc∉ natrecₕ = (λ ()) ∙ suc∉
  suc∉-⇒ₑ suc∉ (unitrecₕ x) = (λ ()) ∙ suc∉
  suc∉-⇒ₑ suc∉ emptyrecₕ = (λ ()) ∙ suc∉
  suc∉-⇒ₑ suc∉ Jₕ = (λ ()) ∙ suc∉
  suc∉-⇒ₑ suc∉ Kₕ = (λ ()) ∙ suc∉
  suc∉-⇒ₑ suc∉ []-congₕ = (λ ()) ∙ suc∉
...
opaque
  ...
  suc∉-⇾ₑ : suc∉ S → ⟨ H , t , ρ , S ⟩ ⇾ₑ ⟨ H′ , t′ , ρ′ , S′ ⟩ → suc∉ S′
  suc∉-⇾ₑ suc∉ (var x) = suc∉
  suc∉-⇾ₑ suc∉ (⇒ₑ x) = suc∉-⇒ₑ suc∉ x

opaque

  -- Not having sucₑ on the stack is preserved by reduction

  suc∉-⇾ₑ* : suc∉ (State.stack s) → s ⇾ₑ* s′ → suc∉ (State.stack s′)
  suc∉-⇾ₑ* suc∉ id = suc∉
  suc∉-⇾ₑ* {s = record{}} suc∉ (_⇨_ {s′ = record{}} x d) =
    suc∉-⇾ₑ* (suc∉-⇾ₑ suc∉ x) d
...
opaque
  ...
  suc∉-⇒ᵥ : suc∉ S → ⟨ H , t , ρ , S ⟩ ⇒ᵥ ⟨ H′ , t′ , ρ′ , S′ ⟩ → suc∉ S′
  suc∉-⇒ᵥ (_ ∙ suc∉) lamₕ = wk-suc∉ suc∉
  suc∉-⇒ᵥ (_ ∙ suc∉) prodˢₕ₁ = suc∉
  suc∉-⇒ᵥ (_ ∙ suc∉) prodˢₕ₂ = suc∉
  suc∉-⇒ᵥ (_ ∙ suc∉) prodʷₕ = wk-suc∉ suc∉
  suc∉-⇒ᵥ (_ ∙ suc∉) zeroₕ = suc∉
  suc∉-⇒ᵥ (_ ∙ suc∉) sucₕ = wk-suc∉ suc∉
  suc∉-⇒ᵥ (_ ∙ suc∉) starʷₕ = suc∉
  suc∉-⇒ᵥ suc∉ (unitrec-ηₕ x) = suc∉
  suc∉-⇒ᵥ (_ ∙ suc∉) rflₕⱼ = suc∉
  suc∉-⇒ᵥ (_ ∙ suc∉) rflₕₖ = suc∉
  suc∉-⇒ᵥ (_ ∙ suc∉) rflₕₑ = suc∉

opaque

  -- Not having sucₑ on the stack is preserved by reduction

  suc∉-⇾ : suc∉ S → ⟨ H , t , ρ , S ⟩ ⇾ ⟨ H′ , t′ , ρ′ , S′ ⟩ → suc∉ S′
  suc∉-⇾ suc∉ (⇾ₑ x) = suc∉-⇾ₑ suc∉ x
  suc∉-⇾ suc∉ (⇒ᵥ x) = suc∉-⇒ᵥ suc∉ x

opaque

  -- Not having sucₑ on the stack is preserved by reduction

  suc∉-⇾* : suc∉ (State.stack s) → s ⇾* s′ → suc∉ (State.stack s′)
  suc∉-⇾* suc∉ id = suc∉
  suc∉-⇾* {s = record{}} suc∉ (_⇨_ {s₂ = record{}} x d) =
    suc∉-⇾* (suc∉-⇾ suc∉ x) d
