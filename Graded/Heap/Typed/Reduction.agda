...
opaque
  ...
  ⊢ₛ-⇒ₑ (A , ⊢H , ⊢t , ⊢S) (unitrecₕ no-η) =
    case inversion-unitrec ⊢t of λ
      (⊢A , ⊢t , ⊢u , B≡At) →
    _ , ⊢H , ⊢t
      , conv (unitrecₑ ⊢u ⊢A no-η) (sym B≡At) ∙ ⊢S
  ⊢ₛ-⇒ₑ (A , ⊢H , ⊢t , ⊢S) emptyrecₕ =
...
opaque
  ...
  ⊢ₛ-⇒ₙ : Δ ⨾ Γ ⊢ s ∷ A → s ⇒ₙ s′ → Δ ⨾ Γ ⊢ s′ ∷ A
  ⊢ₛ-⇒ₙ (A , ⊢H , ⊢t , ⊢S) (sucₕ x) =
    case inversion-suc ⊢t of λ
      (⊢t , A≡ℕ) →
    _ , ⊢H , ⊢t , conv sucₑ (sym A≡ℕ) ∙ ⊢S
  ⊢ₛ-⇒ₙ (A , ⊢H , ⊢t , ⊢e ∙ ⊢S) (numₕ x) =
...
private
  ...
  opaque
    ...
    ⇒ᵥ→⇒/≡ : (b : Bool) → (T b → suc∉ (State.stack s))
           → Δ ⨾ Γ ⊢ s ∷ A → s ⇒ᵥ s′
           → Δ ⊢⟨ b ⟩ ⦅ s ⦆ ⇒/≡ ⦅ s′ ⦆ ∷ A
    ⇒ᵥ→⇒/≡ {A} b prop (B , ⊢H , ⊢t , ⊢e ∙ ⊢S)
      ...
      PE.subst (_ ⊢⟨ b ⟩ ⦅ S ⦆ˢ (wk ρ (lam p t) ∘ wk ρ′ u) [ H ]ₕ ⇒/≡_∷ A) lemma
        (⊢⦅⦆ˢ-subst/cong {u = wk (lift ρ) t [ wk ρ′ u ]₀} b (suc∉-pop ∘→ prop) ⊢S (conv β-⇒ (sym C≡Gu)))}
      where
    ...
    ⇒ᵥ→⇒/≡ b prop (B , ⊢H , ⊢t , ⊢e ∙ ⊢S) prodˢₕ₁ =
      ...
      case Σ-injectivity (sym B≡Σ) of λ
        (F≡F′ , _) →
      ⊢⦅⦆ˢ-subst/cong b (suc∉-pop ∘→ prop) ⊢S (conv (Σ-β₁-⇒ ⊢G ⊢t₁ ⊢t₂ ok)
        (trans F≡F′ (sym C≡F′))) }
    ...
    ⇒ᵥ→⇒/≡ {(k)} {(m)} b prop (B , ⊢H , ⊢t , ⊢e ∙ ⊢S)
      ...
      PE.subst (_ ⊢⟨ b ⟩ ⦅ S ⦆ˢ (prodrec r p q _ _ _) [ H ]ₕ ⇒/≡_∷ _) lemma
        (⊢⦅⦆ˢ-subst/cong {u = wk (liftn ρ′ 2) u [ wk ρ t₁ , wk ρ t₂ ]₁₀}
                       b (suc∉-pop ∘→ prop) ⊢S (conv β-⇒ (sym (C≡ ⊢t))))}
      where
      H₂ : Heap k (2+ m)
      H₂ = H ∙ (∣ S ∣ · r · p , t₁ , ρ) ∙ (∣ S ∣ · r , t₂ , step ρ)
    ...
    ⇒ᵥ→⇒/≡ b prop (B , ⊢H , ⊢t , ⊢e ∙ ⊢S) zeroₕ =
      case inversion-natrecₑ ⊢e of λ {
        (⊢z , ⊢s , ⊢A , PE.refl , B≡) →
      ⊢⦅⦆ˢ-subst/cong b (suc∉-pop ∘→ prop) ⊢S (conv (natrec-zero ⊢A ⊢z ⊢s)
        (sym (B≡ ⊢t))) }
    ...
    ⇒ᵥ→⇒/≡ {(k)} {(m)} b prop (B , ⊢H , ⊢t , ⊢e ∙ ⊢S)
      ...
      case ⊢⦅⦆ˢ-subst/cong {u = wk (liftn ρ′ 2) s [ wk ρ t , nr (wk ρ t) ]₁₀}
             b (suc∉-pop ∘→ prop) ⊢S (conv β-⇒ (sym (B≡ ⊢t))) of λ
        d →
    ...
    ⇒ᵥ→⇒/≡ b prop (B , ⊢H , ⊢t , ⊢e ∙ ⊢S) starʷₕ =
      ...
      ⊢⦅⦆ˢ-subst/cong b (suc∉-pop ∘→ prop) ⊢S (conv (unitrec-β-⇒ ⊢A ⊢u) (sym (C≡A₊ ⊢t)))}
    ...
    ⇒ᵥ→⇒/≡ b prop (B , ⊢H , ⊢rfl , ⊢e ∙ ⊢S) rflₕⱼ =
      ...
      case trans (J-motive-rfl-cong (refl ⊢B) t≡v) (sym (≡B ⊢rfl)) of λ
        ≡B′ →
      ⊢⦅⦆ˢ-subst/cong b (suc∉-pop ∘→ prop) ⊢S (conv (J-β-⇒ t≡v ⊢B ⊢w) ≡B′)}
    ⇒ᵥ→⇒/≡ b prop (B , ⊢H , ⊢rfl , ⊢e ∙ ⊢S) rflₕₖ =
      case inversion-Kₑ ⊢e of λ {
        (⊢v , ⊢B , ok , PE.refl , B′≡)  →
      ⊢⦅⦆ˢ-subst/cong b (suc∉-pop ∘→ prop) ⊢S (conv (K-β-⇒ ⊢B ⊢v ok) (sym (B′≡ ⊢rfl)))}
    ⇒ᵥ→⇒/≡ b prop (B , ⊢H , ⊢rfl , ⊢e ∙ ⊢S) rflₕₑ =
      ...
      case syntacticEqTerm t≡u of λ
        (_ , ⊢t , ⊢u) →
      ⊢⦅⦆ˢ-subst/cong b (suc∉-pop ∘→ prop) ⊢S (conv ([]-cong-β-⇒ t≡u ok) (sym (B′≡ ⊢t ⊢u))) }
...
opaque
  ...
  ⇒ᵥ→⇒ : suc∉ (State.stack s) → Δ ⨾ Γ ⊢ s ∷ A → s ⇒ᵥ s′
       → Δ ⊢ ⦅ s ⦆ ⇒ ⦅ s′ ⦆ ∷ A
  ⇒ᵥ→⇒ suc∉S = ⇒ᵥ→⇒/≡ true (λ _ → suc∉S)
...
opaque
  ...
  ⇒→≡ : Δ ⨾ Γ ⊢ s ∷ A → s ⇾ s′ → Δ ⊢ ⦅ s ⦆ ≡ ⦅ s′ ⦆ ∷ A
  ⇒→≡ {s = record{}} (_ , _ , ⊢t , ⊢S) (⇾ₑ d) =
    PE.subst (_ ⊢ _ ≡_∷ _) (⇾ₑ-⦅⦆-≡ d) (refl (⊢⦅⦆ˢ ⊢S ⊢t))
  ⇒→≡ ⊢s (⇒ᵥ d) =
    ⇒ᵥ→≡ ⊢s d
...
opaque
  ...
  ⊢ˢValue-⇒ᵥ : e PE.≢ sucₑ → Δ ⨾ H ⊢ᵉ e ⟨ wk ρ t ⟩∷ A ↝ B → Δ ⊢ wk ρ t [ H ]ₕ ∷ A → Value t
             → ∃₃ λ m n (s : State _ m n) → ⟨ H , t , ρ , e ∙ S ⟩ ⇒ᵥ s
  -- Ok cases:
  ⊢ˢValue-⇒ᵥ e≢suc (conv ⊢e x) ⊢t v =
    ⊢ˢValue-⇒ᵥ e≢suc ⊢e ⊢t v
  ⊢ˢValue-⇒ᵥ _ ⊢e ⊢t (unitrec-ηᵥ η) =
    _ , _ , _ , unitrec-ηₕ η
  ⊢ˢValue-⇒ᵥ _ (∘ₑ x x₁) ⊢t lamᵥ =
    case inversion-lam-Π ⊢t of λ {
      (_ , PE.refl , _) →
    _ , _ , _ , lamₕ}
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t prodᵥ =
    case inversion-prod-Σ ⊢t of λ {
      (_ , _ , PE.refl , PE.refl , _) →
    _ , _ , _ , prodˢₕ₁}
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t prodᵥ =
    case inversion-prod-Σ ⊢t of λ {
      (_ , _ , PE.refl , PE.refl , _) →
    _ , _ , _ , prodˢₕ₂}
  ⊢ˢValue-⇒ᵥ _ (prodrecₑ x x₁) ⊢t prodᵥ =
    case inversion-prod-Σ ⊢t of λ {
      (_ , _ , PE.refl , PE.refl , _) →
    _ , _ , _ , prodʷₕ}
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t zeroᵥ =
    _ , _ , _ , zeroₕ
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t sucᵥ =
        _ , _ , _ , sucₕ
  ...
  -- Impossible cases:
  ⊢ˢValue-⇒ᵥ e≢suc sucₑ ⊢t v = ⊥-elim (e≢suc PE.refl)
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t lamᵥ =
    case inversion-lam ⊢t of λ
      (_ , _ , _ , _ , _ , Σ≡Π , _) →
    ⊥-elim (Π≢Σⱼ (sym Σ≡Π))
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t lamᵥ =
    case inversion-lam ⊢t of λ
      (_ , _ , _ , _ , _ , Σ≡Π , _) →
    ⊥-elim (Π≢Σⱼ (sym Σ≡Π))
  ⊢ˢValue-⇒ᵥ _ (prodrecₑ x x₁) ⊢t lamᵥ =
    case inversion-lam ⊢t of λ
      (_ , _ , _ , _ , _ , Σ≡Π , _) →
    ⊥-elim (Π≢Σⱼ (sym Σ≡Π))
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t lamᵥ =
    case inversion-lam ⊢t of λ
      (_ , _ , _ , _ , _ , ℕ≡Π , _) →
    ⊥-elim (ℕ≢Π ℕ≡Π)
  ...
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t zeroᵥ =
    ⊥-elim (ℕ≢Σ (sym (inversion-zero ⊢t)))
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t zeroᵥ =
    ⊥-elim (ℕ≢Σ (sym (inversion-zero ⊢t)))
  ...
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t sucᵥ =
    (⊥-elim (ℕ≢Σ (sym (inversion-suc ⊢t .proj₂))))
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t sucᵥ =
    ⊥-elim (ℕ≢Σ (sym (inversion-suc ⊢t .proj₂)))
  ...
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t starᵥ =
    ⊥-elim (Unit≢Σⱼ (sym (inversion-star ⊢t .proj₁)))
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t starᵥ =
    ⊥-elim (Unit≢Σⱼ (sym (inversion-star ⊢t .proj₁)))
  ⊢ˢValue-⇒ᵥ _ (prodrecₑ x x₁) ⊢t starᵥ =
    ⊥-elim (Unit≢Σⱼ (sym (inversion-star ⊢t .proj₁)))
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t starᵥ =
  ...
  ⊢ˢValue-⇒ᵥ _ (∘ₑ x x₁) ⊢t prodᵥ =
    case inversion-prod ⊢t of λ
      (_ , _ , _ , _ , _ , _ , _ , Π≡Σ , _) →
    ⊥-elim (Π≢Σⱼ Π≡Σ)
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t prodᵥ =
    case inversion-prod ⊢t of λ
      (_ , _ , _ , _ , _ , _ , _ , ℕ≡Σ , _) →
    ⊥-elim (ℕ≢Σ ℕ≡Σ)
  ...
  ⊢ˢValue-⇒ᵥ _ (∘ₑ x x₁) ⊢t rflᵥ =
    case inversion-rfl ⊢t of λ
      (_ , _ , _ , _ , Π≡Id) →
    ⊥-elim (Id≢Π (sym Π≡Id))
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t rflᵥ =
    case inversion-rfl ⊢t of λ
      (_ , _ , _ , _ , Σ≡Id) →
    ⊥-elim (Id≢Σ (sym Σ≡Id))
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t rflᵥ =
    case inversion-rfl ⊢t of λ
      (_ , _ , _ , _ , Σ≡Id) →
    ⊥-elim (Id≢Σ (sym Σ≡Id))
  ⊢ˢValue-⇒ᵥ _ (prodrecₑ x x₁) ⊢t rflᵥ =
    case inversion-rfl ⊢t of λ
      (_ , _ , _ , _ , Σ≡Id) →
    ⊥-elim (Id≢Σ (sym Σ≡Id))
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t rflᵥ =
    case inversion-rfl ⊢t of λ
      (_ , _ , _ , _ , ℕ≡Id) →
    ⊥-elim (Id≢ℕ (sym ℕ≡Id))
  ...
  ⊢ˢValue-⇒ᵥ _ (∘ₑ x x₁) ⊢t ΠΣᵥ =
    case inversion-ΠΣ-U ⊢t of λ
      (_ , _ , _ , _ , Π≡U , _) →
    ⊥-elim (U≢Π (sym Π≡U))
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t ΠΣᵥ =
    case inversion-ΠΣ-U ⊢t of λ
      (_ , _ , _ , _ , Σ≡U , _) →
    ⊥-elim (U≢Σ (sym Σ≡U))
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t ΠΣᵥ =
    case inversion-ΠΣ-U ⊢t of λ
      (_ , _ , _ , _ , Σ≡U , _) →
    ⊥-elim (U≢Σ (sym Σ≡U))
  ⊢ˢValue-⇒ᵥ _ (prodrecₑ x x₁) ⊢t ΠΣᵥ =
    case inversion-ΠΣ-U ⊢t of λ
      (_ , _ , _ , _ , Σ≡U , _) →
    ⊥-elim (U≢Σ (sym Σ≡U))
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t ΠΣᵥ =
    case inversion-ΠΣ-U ⊢t of λ
      (_ , _ , _ , _ , ℕ≡U , _) →
    ⊥-elim (U≢ℕ (sym ℕ≡U))
  ...
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t ℕᵥ =
    ⊥-elim (U≢Σ (sym (inversion-ℕ ⊢t)))
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t ℕᵥ =
    ⊥-elim (U≢Σ (sym (inversion-ℕ ⊢t)))
  ⊢ˢValue-⇒ᵥ _ (prodrecₑ x x₁) ⊢t ℕᵥ =
    ⊥-elim (U≢Σ (sym (inversion-ℕ ⊢t)))
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t ℕᵥ =
  ...
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t Unitᵥ =
    ⊥-elim (U≢Σ (sym (inversion-Unit-U ⊢t .proj₁)))
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t Unitᵥ =
    ⊥-elim (U≢Σ (sym (inversion-Unit-U ⊢t .proj₁)))
  ⊢ˢValue-⇒ᵥ _ (prodrecₑ x x₁) ⊢t Unitᵥ =
    ⊥-elim (U≢Σ (sym (inversion-Unit-U ⊢t .proj₁)))
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t Unitᵥ =
  ...
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t Emptyᵥ =
    ⊥-elim (U≢Σ (sym (inversion-Empty ⊢t)))
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t Emptyᵥ =
    ⊥-elim (U≢Σ (sym (inversion-Empty ⊢t)))
  ⊢ˢValue-⇒ᵥ _ (prodrecₑ x x₁) ⊢t Emptyᵥ =
    ⊥-elim (U≢Σ (sym (inversion-Empty ⊢t)))
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t Emptyᵥ =
  ...
  ⊢ˢValue-⇒ᵥ _ (∘ₑ x x₁) ⊢t Idᵥ =
    case inversion-Id-U ⊢t of λ
      (_ , _ , _ , _ , Π≡U) →
    ⊥-elim (U≢Π (sym Π≡U))
  ⊢ˢValue-⇒ᵥ _ (fstₑ x x₁) ⊢t Idᵥ =
    case inversion-Id-U ⊢t of λ
      (_ , _ , _ , _ , Σ≡U) →
    ⊥-elim (U≢Σ (sym Σ≡U))
  ⊢ˢValue-⇒ᵥ _ (sndₑ x x₁) ⊢t Idᵥ =
    case inversion-Id-U ⊢t of λ
      (_ , _ , _ , _ , Σ≡U) →
    ⊥-elim (U≢Σ (sym Σ≡U))
  ⊢ˢValue-⇒ᵥ _ (prodrecₑ x x₁) ⊢t Idᵥ =
    case inversion-Id-U ⊢t of λ
      (_ , _ , _ , _ , Σ≡U) →
    ⊥-elim (U≢Σ (sym Σ≡U))
  ⊢ˢValue-⇒ᵥ _ (natrecₑ x x₁ x₂) ⊢t Idᵥ =
    case inversion-Id-U ⊢t of λ
      (_ , _ , _ , _ , ℕ≡U) →
    ⊥-elim (U≢ℕ (sym ℕ≡U))
