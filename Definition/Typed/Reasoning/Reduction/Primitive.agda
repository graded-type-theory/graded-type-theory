...
finally-⇒∷ : ∀ t A u → Γ ⊢ t ⇒ u ∷ A → Γ ⊢ u ∷ A → Γ ⊢ t ⇒* u ∷ A
finally-⇒∷ _ _ _ t⇒u ⊢u = t⇒u ⇨ id ⊢u

syntax finally-⇒∷ t A u t⇒u ⊢u = t ∷ A ⇒⟨ t⇒u , ⊢u ⟩∎∷ u ∎
...
-- A variant of finally-⇒*∷≡.
  ...
  ∀ t A → u PE.≡ v → Γ ⊢ t ⇒ u ∷ A → Γ ⊢ u ∷ A → Γ ⊢ t ⇒* v ∷ A
...
opaque
  ...
  step-⇐∷ :
    ∀ v A → Γ ⊢ t ⇒* u ∷ A → Γ ⊢ u ⇒ v ∷ A → Γ ⊢ v ∷ A → Γ ⊢ t ⇒* v ∷ A
  step-⇐∷ _ _ t⇒u u⇒v ⊢v = t⇒u ⇨∷* (u⇒v ⇨ id ⊢v)

  syntax step-⇐∷ v A t⇒u u⇒v ⊢v = v ∷ A ⇐⟨ u⇒v , ⊢v ⟩∷ t⇒u
...
finally-⇐∷ : ∀ u A t → Γ ⊢ t ⇒ u ∷ A → Γ ⊢ u ∷ A → Γ ⊢ t ⇒* u ∷ A
finally-⇐∷ _ _ _ t⇒u ⊢u = t⇒u ⇨ id ⊢u

syntax finally-⇐∷ u A t t⇒u ⊢u = u ∷ A ⇐⟨ t⇒u , ⊢u ⟩∎∷ t ∎
...
-- A variant of finally-⇐*∷≡.
  ...
  ∀ v A → u PE.≡ t → Γ ⊢ v ⇒ u ∷ A → Γ ⊢ u ∷ A → Γ ⊢ v ⇒* t ∷ A
finally-⇐∷≡ _ _ PE.refl = finally-⇐ _ _

syntax finally-⇐∷≡ v A u≡t v⇒u ⊢u = v ∷ A ⇐⟨ v⇒u , ⊢u ⟩∎∷≡ u≡t
