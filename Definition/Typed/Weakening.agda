...
mutual
  ...
  wk ρ ⊢Δ (Uⱼ ⊢Γ) = Uⱼ ⊢Δ
  wk ρ ⊢Δ (ΠΣⱼ F ▹ G) = let ρF = wk ρ ⊢Δ F
                        in ΠΣⱼ ρF ▹ wk (lift ρ) (⊢Δ ∙ ρF) G
  ...
  wkTerm ρ ⊢Δ (ΠΣⱼ F ▹ G) = let ρF = wkTerm ρ ⊢Δ F
                            in ΠΣⱼ ρF ▹ wkTerm (lift ρ) (⊢Δ ∙ univ ρF) G
  ...
  wkEq ρ ⊢Δ (ΠΣ-cong F F≡H G≡E) = let ρF = wk ρ ⊢Δ F
                                  in  ΠΣ-cong ρF (wkEq ρ ⊢Δ F≡H)
  ...
  wkEqTerm ρ ⊢Δ (ΠΣ-cong F F≡H G≡E) =
    let ρF = wk ρ ⊢Δ F
  ...
  wkEqTerm {ρ = ρ} [ρ] ⊢Δ (Σ-β₁ {q = q} {G = G} ⊢F ⊢G t u p p≈p′) =
    let ρF = wk [ρ] ⊢Δ ⊢F
        ρG = wk (lift [ρ]) (⊢Δ ∙ ρF) ⊢G
        ρt = wkTerm [ρ] ⊢Δ t
        ρu = wkTerm [ρ] ⊢Δ u
        ρu = PE.subst (λ x → _ ⊢ _ ∷ x) (wk-β G) ρu
    in  Σ-β₁ {q = q} ρF ρG ρt ρu (prodⱼ ρF ρG ρt ρu) p≈p′
  wkEqTerm {ρ = ρ} [ρ] ⊢Δ (Σ-β₂ {q = q} {G = G} ⊢F ⊢G t u p p≈p′) =
    let ρF = wk [ρ] ⊢Δ ⊢F
        ρG = wk (lift [ρ]) (⊢Δ ∙ ρF) ⊢G
        ρt = wkTerm [ρ] ⊢Δ t
    ...
    in  PE.subst (λ x → _ ⊢ _ ≡ _ ∷ x) (PE.sym (wk-β G))
      (Σ-β₂ {q = q} ρF ρG ρt ρu (prodⱼ ρF ρG ρt ρu) p≈p′)
  ...
  wkEqTerm {Δ = Δ} {ρ = ρ} [ρ] ⊢Δ (natrec-suc {p = p} {q = q} {r = r} {n = n} {z = z} {s = s} {F = F} ⊢n ⊢F ⊢z ⊢s) =
    let ρn = wkTerm [ρ] ⊢Δ ⊢n
...
mutual
  ...
  wkRedTerm ρ ⊢Δ (snd-subst {G = G} ⊢F ⊢G t⇒) =
    ...
    in  PE.subst (λ x → _ ⊢ snd _ _ ⇒ snd _ _ ∷ x) (PE.sym (wk-β G))
      (snd-subst ρF ρG ρt⇒)
  wkRedTerm {ρ = ρ} [ρ] ⊢Δ (Σ-β₁ {G = G} ⊢F ⊢G t u ⊢tu p≈p′) =
    let ρF = wk [ρ] ⊢Δ ⊢F
        ρG = wk (lift [ρ]) (⊢Δ ∙ ρF) ⊢G
        ρt = wkTerm [ρ] ⊢Δ t
        ρu = wkTerm [ρ] ⊢Δ u
  ...
  wkRedTerm
    {Δ = Δ} {ρ = ρ} [ρ] ⊢Δ
    (prodrec-β {A = A} {G = G} {u = u} ⊢F ⊢G ⊢A ⊢t ⊢t′ ⊢u p≈p′) =
    let ρF = wk [ρ] ⊢Δ ⊢F
        ρG = wk (lift [ρ]) (⊢Δ ∙ ρF) ⊢G
        ρA = wk (lift [ρ]) (⊢Δ ∙ (ΠΣⱼ ρF ▹ ρG)) ⊢A
        ρt = wkTerm [ρ] ⊢Δ ⊢t
  ...
  wkRedTerm {Δ = Δ} {ρ = ρ} [ρ] ⊢Δ (natrec-suc {p = p} {q = q} {r = r} {n = n} {z = z} {s = s} {F = F} ⊢n ⊢F ⊢z ⊢s) =
    let ρn = wkTerm [ρ] ⊢Δ ⊢n
        ρF = wk (lift [ρ]) (⊢Δ ∙ (ℕⱼ ⊢Δ)) ⊢F
        ρz = PE.subst (λ x → _ ⊢ _ ∷ x) (wk-β F) (wkTerm [ρ] ⊢Δ ⊢z)
