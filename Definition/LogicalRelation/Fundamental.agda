...
mutual
          ...
          , (λ ⊢Δ [σ] →
               ...
               in  transEq (proj₁ (unwrap [A] ⊢Δ [σ])) (proj₁ (unwrap [B₁] ⊢Δ [σ]))
                           ...
                           (irrelevanceEq (proj₁ (unwrap [B₁]₁ ⊢Δ [σ]′))
                                          (proj₁ (unwrap [B₁] ⊢Δ [σ]))
                                          ([B₁≡B] ⊢Δ [σ]′)))
  fundamentalEq (ΠΣ-cong {b = BMΠ} {F = F} {H} {G} {E} ⊢F A≡B A≡B₁ ok)
  ...
  fundamentalEq (ΠΣ-cong {b = BMΣ _} {F = F} {H} {G} {E} ⊢F A≡B A≡B₁ ok)
    with fundamentalEq A≡B | fundamentalEq A≡B₁
  … | [Γ] , [F] , [H] , [F≡H] | [Γ]₁ , [G] , [E] , [G≡E] =
      let [G]′ = S.irrelevance {A = G} [Γ]₁ ([Γ] ∙ [F]) [G]
          [E]′ = S.irrelevanceLift {A = E} {F = F} {H = H} [Γ] [F] [H] [F≡H]
  ...
  fundamentalTerm (ΠΣⱼ {b = BMΠ} {F = F} {G} ⊢F ⊢G ok)
  ...
  fundamentalTerm (ΠΣⱼ {b = BMΣ _} {F = F} {G} ⊢F ⊢G ok)
    with fundamentalTerm ⊢F | fundamentalTerm ⊢G
  ... | [Γ] , [U] , [F]ₜ | [Γ]₁ , [U]₁ , [G]ₜ =
    let [F]   = univᵛ {A = F} [Γ] [U] [F]ₜ
        [U]′  = S.irrelevance {A = U} [Γ]₁ ([Γ] ∙ [F]) [U]₁
  ...
  fundamentalTerm (lamⱼ {F = F} {G} {t} ⊢F ⊢t ok)
      ...
      ,   modelsTermEq [A] [t]″ [u]″
            (λ ⊢Δ [σ] →
               ...
               in  convEqTerm₁ (proj₁ (unwrap [A′]₁ ⊢Δ [σ])) (proj₁ (unwrap [A] ⊢Δ [σ]))
                               ([A′≡A] ⊢Δ [σ]) [t≡u]′)
  fundamentalTermEq
    (ΠΣ-cong {b = BMΠ} {E = E} {F} {G} {H} ⊢F F≡H G≡E ok)
  ...
  fundamentalTermEq
    (ΠΣ-cong {b = BMΣ _} {E = E} {F = F} {G = G} {H = H} ⊢F F≡H G≡E ok)
    with fundamental ⊢F | fundamentalTermEq F≡H | fundamentalTermEq G≡E
  ... | [Γ] , [F] | [Γ]₁ , modelsTermEq [U] [F]ₜ [H]ₜ [F≡H]ₜ
      | [Γ]₂ , modelsTermEq [U]₁ [G]ₜ [E]ₜ [G≡E]ₜ =
    let [U]′  = Uᵛ [Γ]
    ...
    ,   modelsTermEq
          ...
          (Σ-congᵗᵛ {F = F} {G} {H} {E} [Γ] [F] [H] [U]₁′ [U]₂′
                    [F]ₜ′ [G]ₜ′ [H]ₜ′ [E]ₜ′ [F≡H]ₜ′ [G≡E]ₜ′ ok)
  fundamentalTermEq (app-cong {a = a} {b} {f} {g} {F} {G} f≡g a≡b)
  ...
  ... | [Γ] , modelsTermEq [ΠFG] [f] [g] [f≡g]
    ...
    in  [Γ]₁ , modelsTermEq [G[a]]
                            ...
                            (app-congᵛ {F = F} {G} {f} {g} {a} {b}
                                       ...
                                       [b] [a≡b])
  fundamentalTermEq
    (β-red {p = p} {p′ = q} {a = a} {b} {F} {G} ⊢F ⊢G ⊢b ⊢a p≈p′ ok)
  ...
  fundamentalTermEq (natrec-cong {p = p} {q = q} {r = r} {z = z} {z′ = z′} {s = s} {s′ = s′}
                                 {n = n} {n′ = n′} {F = F} {F′ = F′}
                                 ⊢F F≡F′ z≡z′ s≡s′ n≡n′) |
    [Γ]  , [F] , [F′] , [F≡F′] |
  ...
  fundamentalTermEq (natrec-zero {p = p} {q = q} {r = r} {z = z} {s = s} {F = F} ⊢F ⊢z ⊢s) | [Γ] , [F]
    | [Γ]₁ , [F₀] , [z] | [Γ]₂ , [F₊] , [s] =
    let sType = wk1 (F [ suc (var x0) ]↑)
        [Γ]′ = [Γ]₁
        [ℕ]′ = ℕᵛ {l = ¹} [Γ]′
               ...
               let ⊢ℕ = escape (proj₁ (unwrap [ℕ]′ ⊢Δ [σ]))
                   ...
                   ⊢s = PE.subst (λ x → Δ ∙ ℕ ∙ (subst (liftSubst σ) F) ⊢ subst (liftSubstn σ 2) s ∷ x)
                                 ...
                                 (escapeTerm (proj₁ (unwrap [F₊]′ (⊢Δℕ ∙ ⊢F) [σ⇑⇑]))
                                             (proj₁ ([s]′ (⊢Δℕ ∙ ⊢F) [σ⇑⇑])))
               in PE.subst (λ x → Δ ⊢ subst σ (natrec p q r F z s zero)
                                    ⇒ subst σ z ∷ x)
                           (PE.sym (singleSubstLift F zero))
                           (natrec-zero ⊢F ⊢z ⊢s))
                        [F₀] [z]
    in  [Γ]′ , modelsTermEq [F₀] d [z] r
  fundamentalTermEq {Γ = Γ} (natrec-suc {p = p} {q = q} {r = r} {n = n} {z = z} {s = s} {F = F} ⊢n ⊢F ⊢z ⊢s)
  ...
  ... | [Γ] , [ℕ] , [n] | [Γ]₁ , [F] | [Γ]₂ , [F₀] , [z] | [Γ]₃ , [F₊] , [s] =
    let [sucn] = sucᵛ {n = n} [Γ] [ℕ] [n]
        ...
        [natrecₙ] = natrecᵛ {p = p} {r = r} {F = F} {z} {s} {n}
                            [Γ] [ℕ] [F]′ [F₀]′ [F₊]′ [Fₙ]′ [z]′ [s]′ [n]
        [s₊] : Γ ⊩ᵛ⟨ ¹ ⟩ subst (consSubst (consSubst idSubst n) (natrec p q r F z s n)) s ∷ F [ suc n ] / [Γ] / [F[sucn]]
        [s₊] = (λ {k : Nat} {Δ : Con Term k} {σ} (⊢Δ : ⊢ Δ) ([σ] : Δ ⊩ˢ σ ∷ Γ / [Γ] / ⊢Δ) →
                            let [σn] = proj₁ ([n] ⊢Δ [σ])
                                ...
                                [σ₊] = ([σ] , [σn]) , [σnatrecₙ]′
                                [σ₊s] = proj₁ ([s]′ {σ = consSubst (consSubst σ (subst σ n)) (subst σ (natrec p q r F z s n))} ⊢Δ [σ₊])
                                [σ₊s]′ = irrelevanceTerm″ (PE.trans (sucCaseSubstEq F) (PE.sym (singleSubstLift F (suc n))))
                                                          (PE.trans (substVar-to-subst (λ { x0 → PE.refl
                                                                                          ; (x0 +1) → PE.refl
                                                                                          ; (x +1 +1) → PE.refl})
                                                                                       s)
                                                                    (PE.sym (substCompEq {σ = σ} {σ′ = consSubst (consSubst idSubst n) (natrec p q r F z s n)} s)))
                            ...
                            in  [σ₊s]′ , (λ {σ′} [σ′] [σ≡σ′] →
                                let
                                    ...
                                    [σ₊s≡σ′₊s] = proj₂ ([s]′ {σ = consSubst (consSubst σ (subst σ n)) (subst σ (natrec p q r F z s n))}
                                                             ⊢Δ [σ₊])
                                                       {σ′ = consSubst (consSubst σ′ (subst σ′ n)) (subst σ′ (natrec p q r F z s n))}
        ...
        [natrecₛₙ] , [nr≡s₊] = redSubstTermᵛ {A = F [ suc n ]} {natrec p q r F z s (suc n)}
                                            {subst (consSubst (consSubst idSubst n) (natrec p q r F z s n)) s}
                                            [Γ] (λ {k Δ σ} ⊢Δ [σ] →
                                                  ...
                                                  in  PE.subst₂ (λ u A → Δ ⊢ subst σ (natrec p q r F z s (suc n)) ⇒ u ∷ A)
                                                                (PE.trans (doubleSubstComp s (subst σ n) (subst σ (natrec p q r F z s n)) σ)
                                                                          (PE.trans (substVar-to-subst (λ { x0 → PE.refl
                                                                                                          ; (x0 +1) → PE.refl
                                                                                                          ; (x +1 +1) → PE.refl}) s)
                                                                                    (PE.sym (substCompEq s))))
  ...
  fundamentalTermEq
    (Σ-β₁ {p = p} {F = F} {G} {t} {u} ⊢F ⊢G ⊢t ⊢u PE.refl ok)
    with fundamentalTerm ⊢t | fundamental ⊢G | fundamentalTerm ⊢u
  ... | [Γ] , [F] , [t] | [Γ]₁ , [G]₁ | [Γ]₂ , [Gt]₂ , [u]₂ =
    let [G] = S.irrelevance {A = G} [Γ]₁ ([Γ] ∙ [F]) [G]₁
  ...
  fundamentalTermEq
    {Γ = Γ} (Σ-β₂ {p = p} {F = F} {G} {t} {u} ⊢F ⊢G ⊢t ⊢u PE.refl ok)
    with fundamentalTerm ⊢t | fundamental ⊢G | fundamentalTerm ⊢u
  ... | [Γ] , [F] , [t] | [Γ]₁ , [G]₁ | [Γ]₂ , [Gt]₂ , [u]₂ =
    let [G] = S.irrelevance {A = G} [Γ]₁ ([Γ] ∙ [F]) [G]₁
        [Gt] = substS {F = F} {G} [Γ] [F] [G] [t]
  ...
  fundamentalTermEq
    (prodrec-cong {q = q} {r = r} {t = t} {t′} {u} {u′} {F} {G} {A} {A′}
       ⊢F ⊢G ⊢A≡A′ ⊢t≡t′ ⊢u≡u′ ok₁ ok₂)
  ...
  ... | [Γ] , [F] | [Γ]₁ , [G] | [Γ]₂ , [A] , [A′] , [A≡A′]
    ...
    let [G]′ = S.irrelevance {A = G} [Γ]₁ ([Γ] ∙ [F]) [G]
        ...
        [A′₊] = subst↑²S {F = F} {G} {A′} [Γ] [F] [G]′ [Σ]′ [A′]′ ok₁
        [A₊≡A′₊] = subst↑²SEq {F = F} {G} {A} {A′} [Γ] [F] [G]′ [Σ]′
                     [A]′ [A′]′ [A≡A′]′ [A₊]′ ok₁
        ...
        [pr≡pr′] = prodrec-congᵛ
          ...
          [A]′ [A′]′ [A≡A′]′ [A₊]′ [A′₊] [A₊≡A′₊]
          [Aₜ] [t]′ [t′]′ [t≡t′]′ [u]′ [u′]″ [u≡u′]′ ok₂
    in  [Γ] , modelsTermEq [Aₜ] [pr] [pr′]′ [pr≡pr′]
  fundamentalTermEq
    (prodrec-β {q = q} {r = r} {t = t} {t′} {u} {F} {G} {A}
       ⊢F ⊢G ⊢A ⊢t ⊢t′ ⊢u PE.refl ok₁ ok₂)
  ...
  ... | [Γ] , [F] | [Γ]₁ , [G]₁ | [Γ]₂ , [A]₂
    ...
    in  [Γ] , modelsTermEq [A[p]] [pr] [u₊] λ {_} {Δ} {σ} ⊢Δ [σ] →
      let [σF] = proj₁ (unwrap [F] ⊢Δ [σ])
          ...
          red : Δ ⊢ subst σ (prodrec r _ _ A (prod! t t′) u) ⇒ _ ∷ _
          red = prodrec-β ⊢σF ⊢σG ⊢σA ⊢σt ⊢σt′ ⊢σu PE.refl ok₁ ok₂
