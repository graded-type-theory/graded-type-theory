...
data UView {Γ : Cons m n} {l A} : Γ ⊩⟨ l ⟩ A → Set a where
  Uᵣ : ∀ UA → UView (Uᵣ UA)
...
data ℕView {Γ : Cons m n} {l A} : Γ ⊩⟨ l ⟩ A → Set a where
  ℕᵣ : ∀ ℕA → ℕView (ℕᵣ ℕA)
...
data EmptyView {Γ : Cons m n} {l A} : Γ ⊩⟨ l ⟩ A → Set a where
...
data NeutralView {Γ : Cons m n} {l A} : Γ ⊩⟨ l ⟩ A → Set a where
  ne : ∀ neA → NeutralView (ne neA)

ne-view′ :
  Γ ⊢ A ⇒* K → Neutral⁺ (Γ .defs) K → (⊩A : Γ ⊩⟨ l ⟩ A) → NeutralView ⊩A
...
ne-view neK [K] = ne-view′ (id (escape [K])) (ne↑⁺ neK) [K]
...
data IdView {Γ : Cons m n} {l A} : Γ ⊩⟨ l ⟩ A → Set a where
...
data ShapeView (Γ : Cons m n) :
       ∀ l l′ A B → Γ ⊩⟨ l ⟩ A → Γ ⊩⟨ l′ ⟩ B → Set a where
...
goodCases (Unitᵣ D) (ne′ _ D₁ neK K≡K) A≡B =
  ⊥-elim (Unit≢ne neK (whrDet* (A≡B , Unitₙ) (D₁ , ne-whnf neK)))
...
goodCases (ne _) (Unitᵣ′ _ _ D₁ _) (ne₌ M D′ neM K≡M) =
...
... | ()
combine (Unitᵥ UnitA (Unitᵣ _ _ UnitB _)) (ne (ne _ D neK K≡K) neB) =
...
combine (ne neA (ne _ D neK K≡K)) (Unitᵥ (Unitᵣ _ _ UnA _) UnB) =
