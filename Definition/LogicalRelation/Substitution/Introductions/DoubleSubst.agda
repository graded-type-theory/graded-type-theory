...
subst↑²S {n} {Γ} {q} {F} {G} {A} {m} {l} [Γ] [F] [G] [Σ] [A] =
  ...
  let [σF] = proj₁ (unwrap [F] ⊢Δ [σ₋])
      ...
      [σx1x0]′ = irrelevanceTerm′ (wk2-tail-B {σ = σ} (BΣ m q) F G)
                                  σwk[Σ] [σΣ] [σx1x0]
      [σ₊] : Δ ⊩ˢ σ₊ ∷ Γ ∙ (Σ⟨ m ⟩ q ▷ F ▹ G) / [Γ] ∙ [Σ] / ⊢Δ
      [σ₊] = [σ₋] , [σx1x0]′
  ...
  in  [σ₊A]′ , λ {σ′} [σ′] [σ≡σ′] →
    let σ′₊ = consSubst (tail (tail σ′)) (subst σ′ (prod m (var (x0 +1)) (var x0)))
        ...
        [σ′x1x0]′ = irrelevanceTerm′ (wk2-tail-B (BΣ m q) F G)
                                     σ′wk[Σ] [σ′Σ] [σ′x1x0]
        [σ′₊] : Δ ⊩ˢ σ′₊ ∷ Γ ∙ (Σ⟨ m ⟩ q ▷ F ▹ G) / [Γ] ∙ [Σ] / ⊢Δ
        [σ′₊] = [σ′₋] , [σ′x1x0]′
        ...
        [σp≡σ′p]′ = irrelevanceEqTerm′ (wk2-tail-B {σ = σ} (BΣ m q) F G)
                                       σwk[Σ] [σΣ] [σp≡σ′p]
