...
module _
  ...
  natrecTerm {n} (ℕₜ .zero d n≡n zeroᵣ) =
    let [σ0] = irrelevanceTerm {l = l} (ℕᵣ (idRed:*: (ℕⱼ ⊢Δ))) [σℕ]
  ...
  natrecTerm {n} (ℕₜ m d n≡n (ne (neNfₜ neM ⊢m m≡m))) =
    let [σn] = ℕₜ m d n≡n (ne (neNfₜ neM ⊢m m≡m))
...
module _
  ...
  natrec-congTerm {n} {m}
    ...
    let [σ0] = irrelevanceTerm {l = l} (ℕᵣ (idRed:*: (ℕⱼ ⊢Δ))) (proj₁ ([ℕ] ⊢Δ [σ]))
                               (ℕₜ zero (idRedTerm:*: (zeroⱼ ⊢Δ)) n≡n zeroᵣ)
    ...
    in  transEqTerm [σFₙ] eq₁
                    (transEqTerm [σFₙ] natrecN≡M
                                 (convEqTerm₂ [σFₙ] [σ′F′ₘ] [σFₙ≡σ′F′ₘ]
                                              (symEqTerm [σ′F′ₘ] eq₂)))
  -- Refuting cases
  natrec-congTerm [σn] (ℕₜ _ d₁ _ zeroᵣ)
                  (ℕₜ₌ _ _ d₂ d′ t≡u (sucᵣ prop₂)) =
    ⊥-elim (zero≢suc (whrDet*Term (redₜ d₁ , zeroₙ) (redₜ d′ , sucₙ)))
  natrec-congTerm [σn] (ℕₜ n d₁ _ (ne (neNfₜ neK ⊢k k≡k)))
                  (ℕₜ₌ _ _ d₂ d′ t≡u (sucᵣ prop₂)) =
    ⊥-elim (suc≢ne neK (whrDet*Term (redₜ d′ , sucₙ) (redₜ d₁ , ne neK)))
  natrec-congTerm (ℕₜ _ d _ zeroᵣ) [σm]
                  (ℕₜ₌ _ _ d₁ d′ t≡u (sucᵣ prop₂)) =
    ⊥-elim (zero≢suc (whrDet*Term (redₜ d , zeroₙ) (redₜ d₁ , sucₙ)))
  natrec-congTerm (ℕₜ n d _ (ne (neNfₜ neK ⊢k k≡k))) [σm]
                  (ℕₜ₌ _ _ d₁ d′ t≡u (sucᵣ prop₂)) =
    ⊥-elim (suc≢ne neK (whrDet*Term (redₜ d₁ , sucₙ) (redₜ d , ne neK)))

  natrec-congTerm (ℕₜ _ d _ (sucᵣ prop)) [σm]
                  (ℕₜ₌ _ _ d₂ d′ t≡u zeroᵣ) =
    ⊥-elim (zero≢suc (whrDet*Term (redₜ d₂ , zeroₙ) (redₜ d , sucₙ)))
  natrec-congTerm [σn] (ℕₜ _ d₁ _ (sucᵣ prop₁))
                  (ℕₜ₌ _ _ d₂ d′ t≡u zeroᵣ) =
    ⊥-elim (zero≢suc (whrDet*Term (redₜ d′ , zeroₙ) (redₜ d₁ , sucₙ)))
  natrec-congTerm [σn] (ℕₜ n d₁ _ (ne (neNfₜ neK ⊢k k≡k)))
                  (ℕₜ₌ _ _ d₂ d′ t≡u zeroᵣ) =
 ...
 ,   (λ {σ′} [σ′] [σ≡σ′] →
        ...
        in  irrelevanceEqTerm′ eqPrf
              (irrelevance′ (PE.sym (singleSubstComp n′ σ F))
                            (proj₁ ([F]′ ⊢Δ ([σ] , [σn]′))))
              (proj₁ ([Fₙ] ⊢Δ [σ]))
              (natrec-congTerm {F} {F} {z} {z} {s} {s} {σ = σ}
                               ...
                               (reflᵗᵛ {Π ℕ ▹ (F ▹▹ F [ suc (var 0) ]↑)} {s}
                                       [Γ] [F₊] [s])
                               ⊢Δ [σ] [σ′] [σ≡σ′] {n′} {subst σ′ n} [σn]′ [σ′n]′ [σn≡σ′n]))
