  ...
  let [ℕ] = ℕᵛ {l = l} [Γ]
      ...
      [natrec] = [s] ⊢Δ (compSubstS ([Γ] ∙ [ℕ] ∙ [F]) ({![Δ]!} ∙ ℕᵛ {!!} ∙ {!!}) (⊢Δ ∙ ⊢ℕ ∙ ⊢F) ⊢Δ [σ⇑⇑]
                           (consSubstS ({![Δ]!} ∙ ℕᵛ {!!}) (⊢Δ ∙ ⊢ℕ)
                                       ...
                                       (ℕᵛ {![Δ]!}) {![sucm]!})
                           {!!} natrecM))
      [natrec]′ = irrelevanceTerm′ {!proj₁ [natrec]!} {!!} {!!} (proj₁ [natrec])
      -- ((ℕₜ (suc m) (idRedTerm:*: (sucⱼ ⊢m)) n≡n (sucᵣ [m])))
...
natrec-congTerm {Γ = Γ} {Δ = Δ} {F = F} {F′ = F′} {z = z} {z′ = z′} {s = s} {s′ = s′}
  ...
  let n″≡n′ = suc-PE-injectivity (whrDet*Term (redₜ d₁ , sucₙ) (redₜ d , sucₙ))
      ...
      [σ′F′ₘ′] = irrelevance′ (PE.sym (PE.trans (substCompEq F′)
                              ...
                              (proj₁ ([F′] ⊢Δ ([σ′] , [m′])))
      [σ′F′ₛₘ′]′ = {!!}
      -- irrelevance′ (natrecIrrelevantSubst F′ z′ s′ m′ σ′)
      --                           (proj₁ ([F′] ⊢Δ ([σ′] , [σ′sm′])))
      ...
      [σFₙ≡σ′F′ₘ] = transEq [σFₙ] [σ′Fₘ] [σ′F′ₘ] [σFₙ≡σ′Fₘ] [σ′Fₘ≡σ′F′ₘ]
      natrecN = {!!}
      -- appTerm [σFₙ′] [σFₛₙ′]′ [σF₊ₙ′]
      --                   (appTerm [σℕ] [σF₊ₙ′] (proj₁ ([F₊] ⊢Δ [σ]))
      --                            (proj₁ ([s] ⊢Δ [σ])) [n′])
      --                   (natrecTerm {F = F} {z} {s} {n′} {σ = σ}
      --                               [Γ] [F] [F₀] [F₊] [z] [s] ⊢Δ [σ] [n′])
      natrecN′ = {!!}
      -- irrelevanceTerm′ (PE.trans (PE.sym (natrecIrrelevantSubst F z s n′ σ))
      --                                       (PE.sym (singleSubstComp (suc n′) σ F)))
      --                             [σFₛₙ′]′ [σFₛₙ′] natrecN
      natrecM = {!!}
      -- appTerm [σ′F′ₘ′] [σ′F′ₛₘ′]′ [σ′F′₊ₘ′]
  ...
  --                                           (symEqTerm [σ′F′ₘ] eq₂)))
natrec-congTerm {Γ = Γ} {Δ = Δ} {F = F} {F′ = F′} {z = z} {z′ = z′} {s = s}
...
natrec-congᵛ {F = F} {F′} {z} {z′} {s} {s′} {n} {n′} {l = l}
  ...
  let [F]′ = S.irrelevance {A = F} (_∙_ {A = ℕ} [Γ] [ℕ])
      ...
      [Fₙ]′ = irrelevance′ (PE.sym (singleSubstComp (subst σ n) σ F))
                           (proj₁ ([F]′ ⊢Δ ([σ] , [σn]′)))
  in {!!}
  -- irrelevanceEqTerm′ (PE.sym (singleSubstLift F n))
  --                        [Fₙ]′ (proj₁ ([Fₙ] ⊢Δ [σ]))
