...
--   let q , w = redSubst*Term {p = p} n⇒n′ [ℕ] [n′]
--       a , s = redSubstTerm {p = p} x [ℕ] q
...
--       [σn]′ , [σn≡σsm] = redSubst*Term {p = p} (redₜ d) [σℕ] [σsm]
--       [σFₙ]′ = proj₁ ([F] ⊢Δ ([σ] , [σn]))
--       [σFₙ] = irrelevance′ (PE.sym (singleSubstComp n σ F)) [σFₙ]′
--       [σFₛₘ] = irrelevance′ (PE.sym (singleSubstComp (suc m) σ {!F!})) (proj₁ ([F] ⊢Δ ({!!} , {!!})))
...
--       [σFₘ] = irrelevance′ (PE.sym (PE.trans (substCompEq {σ = {![F₊]!}} {σ′ = {!!}} F)
--                                              (substSingletonComp {σ = σ} F)))
--                            (proj₁ ([F] ⊢Δ ([σ] , [m])))
--       [σFₛₘ]′ = irrelevance′ (natrecIrrelevantSubst p r F z s m σ)
--                              (proj₁ ([F] ⊢Δ ([σ] , [σsm])))
--       natrecM = natrecTerm {p = p} {r = r} {F = F} {z = z} {s = s} [Γ] [F] [F₀] [F₊] [z] [s] ⊢Δ [σ] [m]
...
--      -- in' =  proj₁ (redSubst*Term {p = p} reduction [σFₙ] (convTerm₂ [σFₙ] [σFₛₘ] [Fₙ≡Fₛₘ] natrecM′))
...
--       [σ⇑⇑] = liftSubstS [Γℕ] ⊢Δℕ [F] (liftSubstS [Γ] ⊢Δ [ℕ] [σ])
--       ⊢F = escape [σF]
--       ⊢z = PE.subst (λ x → _ ⊢ _ ∷ x) (singleSubstLift F zero)
--                     (escapeTerm (proj₁ ([F₀] ⊢Δ [σ])) (proj₁ ([z] ⊢Δ [σ])))
--       ⊢s = PE.subst (λ x → _ ⊢ subst (liftSubstn σ 2) s ∷ x) (natrecSucCase σ F)
--                     (escapeTerm (proj₁ ([F₊] (⊢Δℕ ∙ ⊢F) [σ⇑⇑]))
--                                 (proj₁ ([s] (⊢Δℕ ∙ ⊢F) [σ⇑⇑]) ))
--       [σ0] = irrelevanceTerm {l = l} (ℕᵣ (idRed:*: (ℕⱼ ⊢Δ))) [σℕ]
--                              (ℕₜ zero (idRedTerm:*: (zeroⱼ ⊢Δ)) n≡n zeroᵣ)
--       [σn]′ , [σn≡σ0] = redSubst*Term {p = p} (redₜ d) (proj₁ ([ℕ] ⊢Δ [σ])) [σ0]
...
--   in  proj₁ (redSubst*Term {p = p} reduction [σFₙ]
...
--       [σ⇑⇑] = liftSubstS {σ = liftSubst σ} [Γℕ] ⊢Δℕ [F] (liftSubstS {σ = σ} [Γ] ⊢Δ [ℕ] [σ])
--       ⊢F = escape [σF]
...
--       [σn]′ , [σn≡σm] = redSubst*Term {p = p} (redₜ d) [σℕ] [σm]
--       [σFₙ]′ = proj₁ ([F] ⊢Δ ([σ] , [σn]))
--       [σFₙ] = irrelevance′ (PE.sym (singleSubstComp n σ F)) [σFₙ]′
