...
module Definition.LogicalRelation.Substitution.Introductions.Natrec (M : Set) {{eqrel : EqRelSet M}} where
open EqRelSet {{...}}
...
-- sucCase₃ : ∀ {Γ : Con (Term M) m} {l} ([Γ] : ⊩ᵛ_ {n = m} Γ)
--            ([ℕ] : Γ ⊩ᵛ⟨ l ⟩ ℕ / [Γ])
...
--            ([ℕ] : Γ ⊩ᵛ⟨ l ⟩ ℕ / [Γ])
--            ([F] : Γ ∙ ℕ ⊩ᵛ⟨ l ⟩ F / [Γ] ∙ [ℕ])
--          → Γ ∙ ℕ ∙ F ⊩ᵛ⟨ l ⟩ (wk1 (F [ suc (var x0) ]↑)) / [Γ] ∙ [ℕ] ∙ [F]
-- sucCase₁ {Γ = Γ} {F} {l} [Γ] [ℕ] [F] = {!!}
...
-- Validity of natrec suc case substitution.
-- sucCaseTerm : ∀ {F s m nrec l} ([Γ] : ⊩ᵛ Γ)
--                 ([ℕ] : Γ ⊩ᵛ⟨ l ⟩ ℕ / [Γ])
--                 ([F] : Γ ∙ ℕ ⊩ᵛ⟨ l ⟩ F / [Γ] ∙ [ℕ])
...
--                 ([m] : Γ ⊩ᵛ⟨ l ⟩ m ∷ ℕ / [Γ] / [ℕ])
--                 ([nrec] : Γ ⊩ᵛ⟨ l ⟩ nrec ∷ F [ m ] / [Γ] / [Fₘ])
--             → Γ ⊩ᵛ⟨ l ⟩ s [ m ,, nrec ] ∷ F [ suc m ] / [Γ] / [Fₛₘ]
-- sucCaseTerm {s = s} {m} {nrec} [Γ] [ℕ] [F] [F₊] [Fₘ] [Fₛₘ] [s] [m] [nrec] ⊢Δ [σ] =
--   let seq = (substConsId s)
--       a = [s] ⊢Δ  ({![σ]!} , (proj₁ {![nrec]!}))
--       b = [s] ((⊢Δ ∙ (escape (proj₁ ([ℕ] ⊢Δ [σ])))) ∙ (escape (proj₁ ([F] {!!} {!!})))) (consSubstS {!!} {!!} (consSubstS [Γ] {!!} {!!} {!!} {!!}) {!!} {!!})
--   in  {!proj₁ a!} , {!proj₁ b!}
--            → Γ ⊩ᵛ⟨ l ⟩ f [ t ] ∷ G [ t ] / [Γ]
...
natrecTerm {Γ = Γ} {Δ = Δ} {p = p} {r = r} {F = F} {z} {s} {n} {σ} {l} [Γ] [F] [F₀] [F₊] [z] [s] ⊢Δ [σ]
  ...
  let [ℕ] = ℕᵛ {l = l} [Γ]
      ...
      [σFₛₘ] = irrelevance′ (PE.sym (singleSubstComp (suc m) σ F)) (proj₁ ([F] ⊢Δ ([σ] , [σsm])))
        -- irrelevance′ (PE.sym (singleSubstComp (suc m) σ F))
        --                     (proj₁ ([F] ⊢Δ ([σ] , [σsm])))
      ...
      -- [σFₘ] = irrelevance′ (PE.sym (PE.trans (substCompEq {σ = {![F₊]!}} {σ′ = {![σF]!}} F)
      --                                        (substSingletonComp {σ = σ} F)))
      --                      (proj₁ ([F] ⊢Δ ([σ] , [m])))
      ...
      plswrk = [s] ⊢Γ (consSubstS {σ = consSubst idSubst {!!}} [Γℕ] ⊢Γ (consSubstS [Γ] ⊢Γ (idSubstS [Γ]) [ℕ] [m]) {![σFₘ]!} natrecM)
      -- [s] (⊢Δℕ ∙ ⊢F)
      --              (consSubstS [Γℕ] (⊢Δℕ ∙ ⊢F)
      --                          (consSubstS [Γ] (⊢Δℕ ∙ ⊢F)
      --                                      {!idSubstS ?!} [ℕ] [σm])
      --                          {![Fₘ]!} natrecM)
      plswrk' = proj₁ plswrk
      -- natrecM′ = sucCaseTerm [Γ] [ℕ] [F] [F₊] {![σFₘ]!} {!!} [s] {![m]!} {!natrecM!}
      natrecM″ = irrelevanceTerm′ {!!} {!!} {!!} plswrk'
      -- ((ℕₜ (suc m) (idRedTerm:*: (sucⱼ ⊢m)) n≡n (sucᵣ [m])))
      ...
      reduction'' = conv* ((natrec-suc ⊢m ⊢F ⊢z ⊢s) ⇨ (id (escapeTerm [σFₛₘ] natrecM″))) (sym (≅-eq (escapeEq [σFₙ] [Fₙ≡Fₛₘ])))
      reduction''' = reduction' ⇨∷* reduction''
     -- in' =  proj₁ (redSubst*Term reduction [σFₙ] (convTerm₂ [σFₙ] [σFₛₘ] [Fₙ≡Fₛₘ] natrecM′))
  in proj₁ (redSubst*Term reduction''' [σFₙ] (convTerm₂ [σFₙ] [σFₛₘ] [Fₙ≡Fₛₘ] natrecM″))
  -- proj₁ (redSubst*Term reduction' [σFₙ] (convTerm₂ [σFₙ] [σFₛₘ] [Fₙ≡Fₛₘ] {!natrecM!}))
  -- (natrecTerm [Γ] [F] [F₀] [F₊] [z] [s] ⊢Δ [σ] [m])))
...
--                     / irrelevance′ (PE.sym (singleSubstComp n σ F))
--                                    (proj₁ ([F] ⊢Δ ([σ] , [σn])))
-- natrec-congTerm {Γ = Γ} {Δ = Δ} {F = F} {F′ = F′} {z = z} {z′ = z′} {s = s} {s′ = s′}
--                 {n = n} {m = m} {σ = σ} {σ′ = σ′} {l = l}
...
--       ⊢z = PE.subst (λ x → _ ⊢ _ ∷ x) (singleSubstLift F zero)
--                     (escapeTerm (proj₁ ([F₀] ⊢Δ [σ])) (proj₁ ([z] ⊢Δ [σ])))
--       ⊢s = PE.subst (λ x → _ ⊢ subst (liftSubst (liftSubst σ)) s ∷ x) (natrecSucCase σ F)
--                     (escapeTerm (proj₁ ([F₊] ? [σ⇑⇑])) (proj₁ ([s] {!⊢Δ!} [σ⇑⇑])))
...
--           ([F]  : Γ ∙ ℕ ⊩ᵛ⟨ l ⟩ F / [Γ] ∙ [ℕ])
--           ([F₀] : Γ ⊩ᵛ⟨ l ⟩ F [ zero ] / [Γ])
--           ([F₊] : Γ ∙ ℕ ∙ F ⊩ᵛ⟨ l ⟩ wk1 (F [ suc (var x0) ]↑) / [Γ])
--           ([Fₙ] : Γ ⊩ᵛ⟨ l ⟩ F [ n ] / [Γ])
--         → Γ ⊩ᵛ⟨ l ⟩ z ∷ F [ zero ] / [Γ] / [F₀]
--         → Γ ∙ ℕ ∙ F ⊩ᵛ⟨ l ⟩ s ∷ wk1 (F [ suc (var x0) ]↑) / [Γ] / [F₊]
--         → ([n] : Γ ⊩ᵛ⟨ l ⟩ n ∷ ℕ / [Γ] / [ℕ])
...
--           ([z′] : Γ ⊩ᵛ⟨ l ⟩ z′ ∷ F′ [ zero ] / [Γ] / [F′₀])
--           ([z≡z′] : Γ ⊩ᵛ⟨ l ⟩ z ≡ z′ ∷ F [ zero ] / [Γ] / [F₀])
--           ([s] : Γ ∙ ℕ ∙ F ⊩ᵛ⟨ l ⟩ s ∷ wk1 (F [ suc (var x0) ]↑) / [Γ] / [F₊])
--           ([s′] : Γ ∙ ℕ ∙ F′ ⊩ᵛ⟨ l ⟩ s′ ∷ wk1 (F′ [ suc (var x0) ]↑) / [Γ]
--                            / [F′₊])
