...
module Definition.LogicalRelation.Substitution.Introductions.Prodrec {a ℓ} (M′ : Setoid a ℓ)
...
prodrec-subst* : ∀ {l t t′ u A F G σ}
               → ([Γ] : ⊩ᵛ Γ)
                 ...
                 (⊢Δ : ⊢ Δ)
                 ([σ] : Δ ⊩ˢ σ ∷ Γ / [Γ] / ⊢Δ)
                 ([t′] : Δ ⊩⟨ l ⟩ t′ ∷ subst σ (Σᵣ q ▷ F ▹ G) / proj₁ (unwrap [Σ] ⊢Δ [σ]))
               → Δ ⊢ t ⇒* t′ ∷ subst σ (Σᵣ q ▷ F ▹ G)
               → Δ ⊢ prodrec p q′ (subst (liftSubst σ) A) t (subst (liftSubstn σ 2) u)
                   ⇒* prodrec p q′ (subst (liftSubst σ) A) t′ (subst (liftSubstn σ 2) u)
                   ∷ subst (liftSubst σ) A [ t ]
prodrec-subst* {Γ = Γ} {q} {Δ = Δ} {p} {q′} {l} {t} {t′} {u} {A} {F} {G} {σ}
...
prodrecTerm : ∀ {F G A t u σ l}
             ...
             → let [Σ] = Σᵛ {F = F} {G = G} {m = Σᵣ} [Γ] [F] [G] in
              ...
              (⊢Δ : ⊢ Δ)
              ([σ] : Δ ⊩ˢ σ ∷ Γ / [Γ] / ⊢Δ)
              ([σt] : Δ ⊩⟨ l ⟩ t ∷ subst σ (Σᵣ q ▷ F ▹ G) / proj₁ (unwrap [Σ] ⊢Δ [σ]))
            → ∃ λ [A[t]] → Δ ⊩⟨ l ⟩ prodrec p q′ (subst (liftSubst σ) A) t (subst (liftSubstn σ 2) u)
                          ∷ subst (liftSubst σ) A [ t ] / [A[t]]
prodrecTerm {Γ = Γ} {q} {Δ = Δ} {r} {F = F} {G} {A} {t} {u} {σ} {l}
            [Γ] [F] [G] [A] [A₊] [u] ⊢Δ [σ]
            [σt]@(Σₜ p t⇒p p≅p (prodₙ {t = p₁} {u = p₂}) ([p₁] , [p₂] , PE.refl)) =
  let [Σ] = Σᵛ {F = F} {G = G} {q = q} {m = Σᵣ} [Γ] [F] [G]
      ...
      [σA] = proj₁ (unwrap [A] {σ = liftSubst σ} (⊢Δ ∙ ⊢σΣ)
                        (liftSubstS {σ = σ} {F = Σᵣ q ▷ F ▹ G} [Γ] ⊢Δ [Σ] [σ]))
      ⊢σA = escape [σA]
      [σA₊] = proj₁ (unwrap [A₊] {σ = liftSubstn σ 2} (⊢Δ ∙ ⊢σF ∙ ⊢σG) [σ⇑⇑])
      ⊢σA₊ = escape [σA₊]
      ...
      A[t]≡A[p] = proj₂ (unwrap [A] {σ = consSubst σ t} ⊢Δ ([σ] , [σt]))
                             {σ′ = consSubst σ (prodᵣ p₁ p₂)}
                             ([σ] , [p]) (reflSubst [Γ] ⊢Δ [σ] , [t≡p])
      A[t]≡A[p]′ = irrelevanceEq″ (PE.sym (singleSubstComp t σ A))
                                  (PE.sym (singleSubstComp (prodᵣ p₁ p₂) σ A))
                                  [σA[t]] [σA[t]]′ A[t]≡A[p]
      ⊢A[t]≡A[p] = ≅-eq (escapeEq [σA[t]] A[t]≡A[p])
      ⊢A[p]≡A[t] = PE.subst₂ (Δ ⊢_≡_) (PE.sym (singleSubstComp (prodᵣ p₁ p₂) σ A))
      ...
      reduction₂ : _ ⊢ _ ⇒* _ ∷ _
      reduction₂ = (prodrec-β {p = r} ⊢σF ⊢σG ⊢σA ⊢p₁ ⊢p₂′ ⊢σu′) ⇨ id ⊢u₊′
...
prodrecTerm {Γ = Γ} {q} {Δ = Δ} {r} {F = F} {G} {A} {t} {u} {σ} {l}
  ...
  let [Σ] = Σᵛ {F = F} {G = G} {q = q} {m = Σᵣ} [Γ] [F] [G]
      ...
      ⊢σu′ = PE.subst (λ x → _ ⊢ subst (liftSubstn σ 2) u ∷ x) (subst-β-prodrec A σ) ⊢σu
      ⊢p = ⊢u-redₜ t⇒p
      [p] : Δ ⊩⟨ l ⟩ p ∷ subst (tail (consSubst σ p)) (Σᵣ q ▷ F ▹ G) / [σΣ]
      ...
      prodrecT′ = neuTerm [σA[p]] (prodrecₙ {p = r} x) (prodrecⱼ ⊢σF ⊢σG ⊢σA ⊢p ⊢σu′)
                          (~-prodrec ⊢σF ⊢σG (escapeEq [σA] (reflEq [σA])) p~p ⊢u≡u′ ≈-refl ≈-refl)
...
prodrecCong : ∀ {l F F′ G G′ A A′ t t′ u u′ σ σ′}
            ...
            → let [Σ] = Σᵛ {F = F} {G = G} {m = Σᵣ} [Γ] [F] [G] in
              ...
              ([A₊] : Γ ∙ F ∙ G ⊩ᵛ⟨ l ⟩ A [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F] ∙ [G])
              ([A′₊] : Γ ∙ F′ ∙ G′ ⊩ᵛ⟨ l ⟩ A′ [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F′] ∙ [G′])
              ([A₊≡A′₊] : Γ ∙ F ∙ G ⊩ᵛ⟨ l ⟩ A [ prodᵣ (var (x0 +1)) (var x0) ]↑² ≡ A′ [ prodᵣ (var (x0 +1)) (var x0) ]↑²  / [Γ] ∙ [F] ∙ [G] / [A₊])
              ([u] : Γ ∙ F ∙ G ⊩ᵛ⟨ l ⟩ u ∷ A [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F] ∙ [G] / [A₊])
              ([u′] : Γ ∙ F′ ∙ G′ ⊩ᵛ⟨ l ⟩ u′ ∷ A′ [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F′] ∙ [G′] / [A′₊])
              ([u≡u′] : Γ ∙ F ∙ G ⊩ᵛ⟨ l ⟩ u ≡ u′ ∷ A [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F] ∙ [G] / [A₊])
            → Δ ⊢ subst σ (Σᵣ q ▷ F ▹ G) ≡ subst σ′ (Σᵣ q ▷ F′ ▹ G′)
            → p ≈ p′
            → q′ ≈ q″
            → (⊢Δ : ⊢ Δ)
              ([σ] : Δ ⊩ˢ σ ∷ Γ / [Γ] / ⊢Δ)
              ...
              ([σt] : Δ ⊩⟨ l ⟩ t ∷ subst σ (Σᵣ q ▷ F ▹ G) / proj₁ (unwrap [Σ] ⊢Δ [σ]))
              ([σt′] : Δ ⊩⟨ l ⟩ t′ ∷ subst σ′ (Σᵣ q ▷ F′ ▹ G′) / proj₁ (unwrap [Σ′] ⊢Δ [σ′]))
              ([σt≡σt′] : Δ ⊩⟨ l ⟩ t ≡ t′ ∷ subst σ (Σᵣ q ▷ F ▹ G) / proj₁ (unwrap [Σ] ⊢Δ [σ]))
            → ∃ λ [A[t]] → Δ ⊩⟨ l ⟩ prodrec p q′ (subst (liftSubst σ) A) t (subst (liftSubstn σ 2) u)
                             ≡ prodrec p′ q″ (subst (liftSubst σ′) A′) t′ (subst (liftSubstn σ′ 2) u′)
                             ∷ subst (liftSubst σ) A [ t ] / [A[t]]
prodrecCong {n} {Γ} {q} {m} {Δ} {p} {p′} {q′} {q″} {l} {F} {F′} {G} {G′}
            {A} {A′} {t} {t′} {u} {u′} {σ} {σ′}
            [Γ] [F] [F′] [F≡F′] [G] [G′] [G≡G′]
            [A] [A′] [A≡A′] [A₊] [A′₊] [A₊≡A′₊] [u] [u′] [u≡u′]
            ⊢Σ≡Σ′ p≈p′ q≈q′ ⊢Δ [σ] [σ′] [σ≡σ′]
            [t]@(Σₜ pₜ dₜ p≅p (prodₙ {t = p₁} {u = p₂}) pProp@(wk[p₁] , wk[p₂] , PE.refl))
            [t′]@(Σₜ rₜ d′ₜ r≅r (prodₙ {t = r₁} {u = r₂}) rProp@(wk[r₁] , wk[r₂] , PE.refl))
  ...
  let [Σ] = Σᵛ {F = F} {G = G} {q = q} {m = Σᵣ} [Γ] [F] [G]
      ...
      [p] : Δ ⊩⟨ _ ⟩ prodᵣ p₁ p₂ ∷ subst σ (Σ q ▷ F ▹ G) / [σΣ]
      [p] = Σₜ pₜ (idRedTerm:*: (⊢u-redₜ dₜ)) p≅p prodₙ pProp
      [r] : Δ ⊩⟨ _ ⟩ prodᵣ r₁ r₂ ∷ subst σ′ (Σ q ▷ F′ ▹ G′) / [σ′Σ′]
      ...
      [A[p]] , [prodrecₚ] = prodrecTerm {Γ = Γ} {q} {p = p} {F = F} {G} {A} {t = prodᵣ p₁ p₂} {u}
                                        [Γ] [F] [G] [A] [A₊] [u] ⊢Δ [σ] [p]
      _ , [t≡p] = redSubst*Term (redₜ dₜ) [σΣ] [p]
      [At≡Ap] = proj₂ (unwrap [A] {σ = consSubst σ t} ⊢Δ ([σ] , [t]))
                      {σ′ = consSubst σ (prodᵣ p₁ p₂)} ([σ] , [p]) ([σ≡σ] , [t≡p])
      ...
      [A′[r]] , [prodrecᵣ] = prodrecTerm {Γ = Γ} {q} {p = p′} {F = F′} {G′} {A′} {t = prodᵣ r₁ r₂} {u′}
                                         [Γ] [F′] [G′] [A′] [A′₊] [u′] ⊢Δ [σ′] [r]
      [t′≡r] = proj₂ (redSubst*Term d⁗ [σ′Σ′] [r])
      [A′t′≡A′r] = proj₂ (unwrap [A′] {σ = consSubst σ′ t′} ⊢Δ ([σ′] , [t′]))
                         {σ′ = consSubst σ′ (prodᵣ r₁ r₂)} ([σ′] , [r]) ([σ′≡σ′] , [t′≡r])
      ...
      red₃ : _ ⊢ _ ⇒ _ ∷ _
      red₃ = prodrec-β {p = p} ⊢σF ⊢σG ⊢σA ⊢p₁ ⊢p₂ ⊢σu₁
      [p₂]′ = irrelevanceTerm′ (singleSubstComp p₁ σ G)
                               [σGp₁] [σGp₁]′ [p₂]
      σ₊ = consSubst (consSubst σ p₁) p₂
      [σ₊] = ([σ] , [p₁]) , [p₂]′
      [σ₊A₊] = proj₁ (unwrap [A₊] {σ = σ₊} ⊢Δ [σ₊])
      [σ₊A₊]′ = irrelevance′ (PE.sym (singleSubstComp (prodᵣ p₁ p₂) σ A))
                             (proj₁ (unwrap [A] {σ = consSubst σ (prodᵣ p₁ p₂)} ⊢Δ ([σ] , [p])))
      [σ₊u₊] = proj₁ ([u] {σ = σ₊} ⊢Δ [σ₊])
      ...
      [σ′₊] = ([σ′] , [r₁]) , [r₂]′
      [σ′₊A′₊] = proj₁ (unwrap [A′₊] {σ = σ′₊} ⊢Δ [σ′₊])
      [σ′₊A′₊]′ = irrelevance′ (PE.sym (singleSubstComp (prodᵣ r₁ r₂) σ′ A′))
                               (proj₁ (unwrap [A′] {σ = consSubst σ′ (prodᵣ r₁ r₂)} ⊢Δ ([σ′] , [r])))
      [σ′₊u′₊] = proj₁ ([u′] {σ = σ′₊} ⊢Δ [σ′₊])
      ...
      [σ′₊]′ = ([σ′] , [r₁]′) , [r₂]″
      _ , t≡p₁ , u≡p₂ = prod-PE-injectivity tu≡p
      _ , t′≡r₁ , u′≡r₂ = prod-PE-injectivity tu′≡r
      ...
      [A′[r]]′ = proj₁ (unwrap [A′] {σ = consSubst σ′ (prodᵣ r₁ r₂)} ⊢Δ ([σ′] , [r]))
      ...
      [σAt≡σ′A′r]′ = irrelevanceEq″ (PE.sym (singleSubstComp t σ A))
                                    (PE.sym (singleSubstComp (prodᵣ r₁ r₂) σ′ A′))
                                    [A[t]]′ [A[t]] [σAt≡σ′A′r]
...
prodrecCong {n} {Γ} {q} {m} {Δ} {p} {p′} {q′} {q″} {l} {F} {F′} {G} {G′}
  ...
  let [Σ] = Σᵛ {F = F} {G = G} {q = q} {m = Σᵣ} [Γ] [F] [G]
      ...
      var0 = conv var0′ (PE.subst (λ (x : Term (1+ m)) → Δ ∙ σΣ′ ⊢ U.wk (step id) σΣ′ ≡ x)
                                  ...
                                  (≅-eq wk⊢σΣ′≡σ′Σ′))
      [var0] : Δ ∙ σΣ′ ⊩⟨ _ ⟩ var x0
               ∷ subst (wk1Subst σ′) (Σ q ▷ F′ ▹ G′) / proj₁ (unwrap [Σ′] (⊢Δ ∙ ⊢σΣ′) [↑σ′]′)
      [var0] = neuTerm {Γ = Δ ∙ σΣ′}
      ...
      σprᵣ~σ′prᵣ = ~-prodrec {q = q} {p = p′} {A = subst (liftSubst σ) A′}
                            ...
                            ⊢σF′ ⊢σG′ ⊢σA′≡σ′A′
                            (PE.subst (λ (x : Term m) → Δ ⊢ x ~ x ∷ subst σ (Σᵣ q ▷ F′ ▹ G′)) r≡r′ r~r)
                            ⊢σu′≅σ′u′ ≈-refl ≈-refl
      ...
      _ , [t≡p] = redSubst*Term (redₜ dₜ) [σΣ] [p]
      [A[p]] , [prₚ] = prodrecTerm {p = p} {F = F} {G} {A} {pₜ} {u}
                                   [Γ] [F] [G] [A] [A₊] [u] ⊢Δ [σ] [p]
      [At≡Ap]′ = proj₂ (unwrap [A] {σ = consSubst σ t} ⊢Δ ([σ] , [t]))
      ...
      [A′[r]] , [prᵣ] = prodrecTerm {p = p′} {F = F′} {G′} {A′} {rₜ} {u′}
                                    [Γ] [F′] [G′] [A′] [A′₊] [u′] ⊢Δ [σ′] [r]
      ...
      [r]″ = convTerm₂ [σΣ′] [σ′Σ′] [σΣ′≡σ′Σ′] [r]
      [p≡r] : Δ ⊩⟨ l ⟩ pₜ ≡ rₜ ∷ subst σ (Σᵣ q ▷ F ▹ G) / [σΣ]
      [p≡r] = Σₜ₌ pₜ rₜ (idRedTerm:*: (⊢u-redₜ dₜ)) (idRedTerm:*: (conv (⊢u-redₜ d′ₜ) (sym ⊢Σ≡Σ′)))
                  (ne xₜ) (ne yₜ) (PE.subst₂ (λ (x y : Term m) → Δ ⊢ x ≅ y ∷ subst σ (Σᵣ q ▷ F ▹ G)) p≡p′ r≡r′ p≅r)
                  [p] [r]′ (PE.subst₂ (λ (x y : Term m) → Δ ⊢ x ~ y ∷ subst σ (Σᵣ q ▷ F ▹ G)) p≡p′ r≡r′ p~r)
...
prodrecᵛ : ∀ {F G A t u l}
         → ([Γ] : ⊩ᵛ Γ)
           ...
           ([u] : Γ ∙ F ∙ G ⊩ᵛ⟨ l ⟩ u ∷ A [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F] ∙ [G] / [A₊])
         → Γ ⊩ᵛ⟨ l ⟩ prodrec p q′ A t u ∷ A [ t ] / [Γ] / [Aₜ]
prodrecᵛ {n} {Γ} {q} {p} {q′} {F} {G} {A} {t} {u} {l}
         [Γ] [F] [G] [Σ] [A] [A₊] [Aₜ] [t] [u]
         {k} {Δ} {σ} ⊢Δ [σ] =
  let [Σ]′ = Σᵛ {F = F} {G = G} {q = q} {m = Σᵣ} [Γ] [F] [G]
      [A]′ = S.irrelevance {A = A} (_∙_ {A = Σᵣ q ▷ F ▹ G} [Γ] [Σ]) ([Γ] ∙ [Σ]′) [A]
      [σt] = proj₁ ([t] ⊢Δ [σ])
      [σt]′ = irrelevanceTerm (proj₁ (unwrap [Σ] ⊢Δ [σ])) (proj₁ (unwrap [Σ]′ ⊢Δ [σ])) [σt]
      -- TODO: Slow!
      [A[t]] , [σpr] = prodrecTerm {p = p} {F = F} {G} {A} {subst σ t} {u} {σ} [Γ] [F] [G] [A]′ [A₊] [u] ⊢Δ [σ] [σt]′
      [σAₜ] = proj₁ (unwrap [Aₜ] ⊢Δ [σ])
  in  irrelevanceTerm′ (PE.sym (singleSubstLift A t)) [A[t]] [σAₜ] [σpr]
      , λ {σ′} [σ′] [σ≡σ′] →
        let A₊ = A [ prodᵣ (var (x0 +1)) (var x0) ]↑²
            [σ′t] = proj₁ ([t] ⊢Δ [σ′])
            ...
            -- TODO: Slow!
            [A[t]]′ , [σpr≡σ′pr] = prodrecCong {q = q} {p = p} {p} {F = F} {F} {G} {G} {A} {A}
                                               {subst σ t} {subst σ′ t} {u} {u} {σ} {σ′}
                                               [Γ] [F] [F] (reflᵛ {A = F} [Γ] [F])
                                               [G] [G] (reflᵛ {Γ = Γ ∙ F} {A = G} ([Γ] ∙ [F]) [G])
                                               [A]′ [A]′ (reflᵛ {Γ = Γ ∙ (Σᵣ q ▷ F ▹ G)} {A = A} ([Γ] ∙ [Σ]′) [A]′)
                                               [A₊] [A₊] (reflᵛ {Γ = Γ ∙ F ∙ G} {A = A₊} ([Γ] ∙ [F] ∙ [G]) [A₊])
                                               [u] [u] (reflᵗᵛ {Γ = Γ ∙ F ∙ G} {A = A₊} {t = u} ([Γ] ∙ [F] ∙ [G]) [A₊] [u])
                                               ⊢σΣ≡σ′Σ ≈-refl ≈-refl ⊢Δ [σ] [σ′] [σ≡σ′] [σt]′ [σ′t]′ [σt≡σ′t]′
...
prodrec-congᵛ : ∀ {F F′ G G′ A A′ t t′ u u′ l}
              → ([Γ] : ⊩ᵛ Γ)
                ...
                ([t′] : Γ ⊩ᵛ⟨ l ⟩ t′ ∷ Σᵣ q ▷ F′ ▹ G′ / [Γ] / [Σ′])
                ([t≡t′] : Γ ⊩ᵛ⟨ l ⟩ t ≡ t′ ∷ Σᵣ q ▷ F ▹ G / [Γ] / [Σ])
                ([u] : Γ ∙ F ∙ G ⊩ᵛ⟨ l ⟩ u ∷ A [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F] ∙ [G] / [A₊])
                ([u′] : Γ ∙ F′ ∙ G′ ⊩ᵛ⟨ l ⟩ u′ ∷ A′ [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F′] ∙ [G′] / [A′₊])
                ([u≡u′] : Γ ∙ F ∙ G ⊩ᵛ⟨ l ⟩ u ≡ u′ ∷ A [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F] ∙ [G] / [A₊])
              → p ≈ p′
              → q′ ≈ q″
              → Γ ⊩ᵛ⟨ l ⟩ prodrec p q′ A t u ≡ prodrec p′ q″ A′ t′ u′ ∷ A [ t ] / [Γ] / [Aₜ]
prodrec-congᵛ {Γ = Γ} {q} {p} {p′} {F = F} {F′} {G} {G′} {A} {A′} {t} {t′} {u} {u′}
              [Γ] [F] [F′] [F≡F′] [G] [G′] [G≡G′] [Σ] [Σ′] [Σ≡Σ′]
              [A] [A′] [A≡A′] [A₊] [A′₊] [A₊≡A′₊] [Aₜ]
              [t] [t′] [t≡t′] [u] [u′] [u≡u′] p≈p′ q≈q′ {k} {Δ} {σ} ⊢Δ [σ]  =
  let [Σ]′ = Σᵛ {F = F} {G = G} {q = q} {m = Σᵣ} [Γ] [F] [G]
      [Σ′]′ = Σᵛ {F = F′} {G = G′} {q = q} {m = Σᵣ} [Γ] [F′] [G′]
      [A]′ = S.irrelevance {A = A} (_∙_ {A = Σᵣ q ▷ F ▹ G} [Γ] [Σ]) ([Γ] ∙ [Σ]′) [A]
      [A′]′ = S.irrelevance {A = A′} (_∙_ {A = Σᵣ q ▷ F′ ▹ G′} [Γ] [Σ′]) ([Γ] ∙ [Σ′]′) [A′]
      -- TODO: Slow!
      [A≡A′]′ = S.irrelevanceEq {A = A} {B = A′} (_∙_ {A = Σᵣ q ▷ F ▹ G} [Γ] [Σ]) (_∙_ {A = Σᵣ q ▷ F ▹ G} [Γ] [Σ]′) [A] [A]′ [A≡A′]
      [t]′ = S.irrelevanceTerm {A = Σᵣ q ▷ F ▹ G} {t = t} [Γ] [Γ] [Σ] [Σ]′ [t]
      [t′]′ = S.irrelevanceTerm {A = Σᵣ q ▷ F′ ▹ G′} {t = t′} [Γ] [Γ] [Σ′] [Σ′]′ [t′]
      [σt] = proj₁ ([t]′ ⊢Δ [σ])
      [σt′] = proj₁ ([t′]′ ⊢Δ [σ])
      [σt≡σt′] = [t≡t′] ⊢Δ [σ]
