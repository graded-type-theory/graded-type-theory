...
private
  ...
  ≤ᶜ𝟘ᶜ : (∀ p → p ≤ 𝟘) → (γ : Conₘ n) → γ ≤ᶜ 𝟘ᶜ
  ≤ᶜ𝟘ᶜ _  ε       = ε
  ≤ᶜ𝟘ᶜ ≤𝟘 (_ ∙ _) = ≤ᶜ𝟘ᶜ ≤𝟘 _ ∙ ≤𝟘 _

  +ᶜ≤ᶜ∧ᶜ :
    (∀ p q → p + q ≤ p ∧ q) →
    (γ δ : Conₘ n) → γ +ᶜ δ ≤ᶜ γ ∧ᶜ δ
  +ᶜ≤ᶜ∧ᶜ _   ε       ε       = ε
  +ᶜ≤ᶜ∧ᶜ +≤∧ (_ ∙ _) (_ ∙ _) = +ᶜ≤ᶜ∧ᶜ +≤∧ _ _ ∙ +≤∧ _ _
...
prodₚᵣₘ +≤∧ ▸t ▸u = sub (prodₚₘ ▸t ▸u) (+ᶜ≤ᶜ∧ᶜ +≤∧ _ _)
...
prodᵣₚₘ ∧≤+ ▸t ▸u = sub (prodᵣₘ ▸t ▸u) (∧ᶜ≤ᶜ+ᶜ ∧≤+ _ _)
...
prodrecₚₘ {p = p} {γ = γ} {m = m} {r = r} {δ = δ} p≤𝟙⊎𝟘ᵐ ▸t ▸u = sub
  (doubleSubstₘ-lemma₁ ▸u
     (sndₘ ▸t)
     (fstₘ (m ᵐ· r) (▸-cong (lemma m) (▸-· ▸t)) (ᵐ·-·-assoc m) p≤𝟙⊎𝟘ᵐ))
    (begin
...
𝟘≰𝟙→fstᵣ′-𝟘-not-ok {γ = γ} {p = p} {q = q} {A = A} {t = t} 𝟘≰𝟙 =
  γ ▸[ 𝟙ᵐ ] fstᵣ′ 𝟘 p q A t  →⟨ proj₁ ∘→ proj₂ ∘→ proj₂ ∘→ proj₂ ∘→ proj₂ ∘→ proj₂ ∘→ proj₂ ∘→ inv-usage-fstᵣ′-𝟙ᵐ ⟩
...
fstᵣⱼ :
  Γ ⊢ t ∷ Σᵣ p , q ▷ A ▹ B →
  Γ ⊢ fstᵣ p A t ∷ A
fstᵣⱼ {Γ = Γ} {t = t} {p = p} {q = q} {A = A} {B = B} ⊢t =              $⟨ Σ⊢wk1 ⊢B , 1∷wk1[1,0] ⊢B ⟩
  (Γ ∙ (Σᵣ p , q ▷ A ▹ B) ⊢ wk1 A) ×
  Γ ∙ A ∙ B ⊢ var (x0 +1) ∷ wk1 A [ prodᵣ p (var (x0 +1)) (var x0) ]↑²  →⟨ (λ (hyp₁ , hyp₂) → prodrecⱼ ⊢A ⊢B hyp₁ ⊢t hyp₂) ⟩
...
fstᵣ-β-⇒ {Γ = Γ} {A = A} {B = B} {t = t} {u = u} {p = p} ⊢B ⊢t ⊢u =     $⟨ Σ⊢wk1 ⊢B , 1∷wk1[1,0] ⊢B ⟩
  (Γ ∙ (Σᵣ p , 𝟘 ▷ A ▹ B) ⊢ wk1 A) ×
  Γ ∙ A ∙ B ⊢ var (x0 +1) ∷ wk1 A [ prodᵣ p (var (x0 +1)) (var x0) ]↑²  →⟨ (λ (hyp₁ , hyp₂) → prodrec-β {q = 𝟘} ⊢A ⊢B hyp₁ ⊢t ⊢u hyp₂ PE.refl) ⟩
...
fstᵣ-subst
  {Γ = Γ} {A = A} {B = B} {t₁ = t₁} {t₂ = t₂} {p = p} {q = q} ⊢B t₁⇒t₂ =
                                                                        $⟨ Σ⊢wk1 ⊢B , 1∷wk1[1,0] ⊢B ⟩
  (Γ ∙ (Σᵣ p , q ▷ A ▹ B) ⊢ wk1 A) ×
  Γ ∙ A ∙ B ⊢ var (x0 +1) ∷ wk1 A [ prodᵣ p (var (x0 +1)) (var x0) ]↑²  →⟨ (λ (hyp₁ , hyp₂) → prodrec-subst ⊢A ⊢B hyp₁ hyp₂ t₁⇒t₂) ⟩
...
fstᵣ-β-≡ :
  ...
  Γ ⊢ u ∷ B [ t ] →
  Γ ⊢ fstᵣ p A (prodᵣ p t u) ≡ t ∷ A
fstᵣ-β-≡ ⊢B ⊢t ⊢u = subsetTerm (fstᵣ-β-⇒ ⊢B ⊢t ⊢u)
...
fstᵣ-cong
  ...
  Γ ∙ A₁ ∙ B₁ ⊢
    var (x0 +1) ∷
    wk1 A₁ [ prodᵣ p (var (x0 +1)) (var x0) ]↑²    →⟨ (λ (hyp₁ , hyp₂) → prodrec-cong ⊢A₁ ⊢B₁ hyp₁ t₁≡t₂ (refl hyp₂)) ⟩
...
private
  ...
  ⊢≡[fstᵣ-0]↑[1,0]↑² :
    ...
    Γ ∙ A ∙ B ⊢
      ...
      B [ fstᵣ p (wk1 A) (var x0) ]↑
        [ prodᵣ p (var (x0 +1)) (var x0) ]↑²
  ⊢≡[fstᵣ-0]↑[1,0]↑² {Γ = Γ} {A = A} {B = B} {p = p} ⊢B =            $⟨ substRefl (⊢ˢwk1Subst-wk1Subst-idSubst ⊢B) , lemma ⟩
    Γ ∙ A ∙ B ⊢ˢ
    ...
    lemma =                                                       $⟨ ⊢wk1 ⊢B ⊢B ⟩
      ...
      (Γ ∙ A ∙ B ⊢ var x0 ∷ wk1 (wk1 B) [ var (x0 +1) ])          →⟨ uncurry (uncurry ∘→ fstᵣ-β-≡) ⟩
  ...
  ⊢[fstᵣ-0]↑ ⊢B =
    syntacticEq (⊢[fstᵣ-0]↑≡[fstᵣ-0]↑ (refl ⊢A) (refl ⊢B)) .proj₁
    where
    ⊢A = case wf ⊢B of λ where
           (_ ∙ ⊢A) → ⊢A
  ...
  ⊢0∷[fstᵣ-0]↑[1,0]↑² :
    ...
    Γ ∙ A ∙ B ⊢
      ...
      B [ fstᵣ p (wk1 A) (var x0) ]↑
        [ prodᵣ p (var (x0 +1)) (var x0) ]↑²
  ⊢0∷[fstᵣ-0]↑[1,0]↑² {Γ = Γ} {A = A} {B = B} {p = p} ⊢B =
                                              $⟨ var (wf ⊢B ∙ ⊢B) here ⟩

    Γ ∙ A ∙ B ⊢ var x0 ∷ wk1 B                →⟨ flip conv (⊢≡[fstᵣ-0]↑[1,0]↑² ⊢B) ⟩
...
sndᵣⱼ {Γ = Γ} {t = t} {p = p} {q = q} {A = A} {B = B} ⊢t =   $⟨ prodrecⱼ ⊢A ⊢B (⊢[fstᵣ-0]↑ ⊢B) ⊢t (⊢0∷[fstᵣ-0]↑[1,0]↑² ⊢B) ⟩
...
sndᵣ-subst
  {Γ = Γ} {t₁ = t₁} {t₂ = t₂} {p = p} {q = q} {A = A} {B = B} t₁⇒t₂ =
                                           $⟨ prodrec-subst ⊢A ⊢B (⊢[fstᵣ-0]↑ ⊢B) (⊢0∷[fstᵣ-0]↑[1,0]↑² ⊢B) t₁⇒t₂ ⟩
...
sndᵣ-β-≡ :
  ...
  Γ ⊢ u ∷ B [ t ] →
  Γ ⊢ sndᵣ p q A B (prodᵣ p t u) ≡ u ∷ B [ fstᵣ p A (prodᵣ p t u) ]
sndᵣ-β-≡ ⊢B ⊢t ⊢u = subsetTerm (sndᵣ-β-⇒ ⊢B ⊢t ⊢u)
...
sndᵣ-cong
  ...
  {p = p} {q = q} A₁≡A₂ B₁≡B₂ t₁≡t₂ =          $⟨ prodrec-cong ⊢A ⊢B (⊢[fstᵣ-0]↑≡[fstᵣ-0]↑ A₁≡A₂ B₁≡B₂)
                                                    t₁≡t₂ (refl (⊢0∷[fstᵣ-0]↑[1,0]↑² ⊢B)) ⟩
  Γ ⊢ sndᵣ p q A₁ B₁ t₁ ≡ sndᵣ p q A₂ B₂ t₂ ∷
    B₁ [ fstᵣ p (wk1 A₁) (var x0) ]↑ [ t₁ ]    →⟨ flip conv (⊢≡[fstᵣ] ⊢t₁) ⟩
