...
Conₘ-interchange (natrecₘ {γ = γ} {δ = δ} {p = p} {r = r} {η = η} γ▸z δ▸s η▸n)
  ...
  eq = let γ'  = γ , x ≔ (γ′ ⟨ x ⟩)
       ...
       in begin
          nrᶜ γ' (δ' +ᶜ p ·ᶜ η') r
            ≡˘⟨ cong (λ x₁ → nrᶜ γ' x₁ r) (cong (δ' +ᶜ_) (update-distrib-·ᶜ η p (η′ ⟨ x ⟩) x)) ⟩
          nrᶜ γ' (δ' +ᶜ pη') r
            ≡˘⟨ cong (λ x₁ → nrᶜ γ' x₁ r) (cong (_ +ᶜ_) (cong (_ , x ≔_) (lookup-distrib-·ᶜ η′ p x))) ⟩
          nrᶜ γ' (δ' +ᶜ ((p ·ᶜ η) , x ≔ ((p ·ᶜ η′) ⟨ x ⟩))) r
            ≡˘⟨ cong (λ x₁ → nrᶜ γ' x₁ r) (update-distrib-+ᶜ δ (p ·ᶜ η) (δ′ ⟨ x ⟩) ((p ·ᶜ η′) ⟨ x ⟩) x) ⟩
          nrᶜ γ' ((δ +ᶜ p ·ᶜ η) , x ≔ (δ′ ⟨ x ⟩ + (p ·ᶜ η′) ⟨ x ⟩)) r
            ≡˘⟨ cong (λ x₁ → nrᶜ γ' x₁ r) (cong (_ , x ≔_) (lookup-distrib-+ᶜ δ′ (p ·ᶜ η′) x)) ⟩
          nrᶜ γ' ((δ +ᶜ p ·ᶜ η) , x ≔ ((δ′ +ᶜ p ·ᶜ η′) ⟨ x ⟩)) r
            ≡˘⟨ update-distrib-nrᶜ γ (δ +ᶜ p ·ᶜ η) r (γ′ ⟨ x ⟩) ((δ′ +ᶜ p ·ᶜ η′) ⟨ x ⟩) x ⟩
          nrᶜ γ (δ +ᶜ p ·ᶜ η) r , x ≔ nr (γ′ ⟨ x ⟩) ((δ′ +ᶜ p ·ᶜ η′) ⟨ x ⟩) r
            ≡˘⟨ cong (_ , x ≔_) (lookup-distrib-nrᶜ γ′ (δ′ +ᶜ p ·ᶜ η′) r x) ⟩
          nrᶜ γ (δ +ᶜ (p ·ᶜ η)) r , x ≔ (nrᶜ γ′ (δ′ +ᶜ (p ·ᶜ η′)) r ⟨ x ⟩) ∎
...
usage-upper-bound (natrecₘ {z = z} {s = s} {n = n} γ▸z δ▸s η▸n) =
  nrᶜ-monotone γ≤γ′ (+ᶜ-monotone (tailₘ-monotone (tailₘ-monotone δ≤δ′)) (·ᶜ-monotoneʳ η≤η′)) ≤-refl
  where
