...
substₘ-lemma Ψ σ Ψ▶σ (natrecₘ {γ} {p = p} {r = r} {δ} γ▸z γ▸s δ▸n r≤0) = sub
  (natrecₘ γ▸z′ (sub γ▸s′ eq) δ▸n′ r≤0)
  eq′
...
-- subst₂ _▸_ eq refl (natrecₘ γ▸z′ γ▸s″ δ▸n′ r≤0)
  ...
  eq = R₁.begin
    Ψ *> γ ∙ p ∙ r
      R₁.≈⟨ (≈ᶜ-sym (liftSubstₘ-app Ψ γ p)) ∙ ≈-refl ⟩
    (wk1Substₘ Ψ ⊙ (𝟘ᶜ ∙ 𝟙)) *> (γ ∙ p) ∙ r
      R₁.≈⟨ ≈ᶜ-sym (liftSubstₘ-app (wk1Substₘ Ψ ⊙ (𝟘ᶜ ∙ 𝟙)) (γ ∙ p) r) ⟩
    ((wk1Substₘ (wk1Substₘ Ψ) ⊙ (𝟘ᶜ ∙ 𝟙 ∙ 𝟘) ⊙ (𝟘ᶜ ∙ 𝟙)) *> (γ ∙ p ∙ r))
      R₁.≡⟨⟩
    r ·ᶜ (𝟘ᶜ ∙ 𝟘 ∙ 𝟙) +ᶜ p ·ᶜ (𝟘ᶜ ∙ 𝟙 ∙ 𝟘) +ᶜ ((wk1Substₘ (wk1Substₘ Ψ)) *> γ)
      R₁.≡⟨⟩
    (r ·ᶜ 𝟘ᶜ ∙ r · 𝟘 ∙ r · 𝟙) +ᶜ (p ·ᶜ 𝟘ᶜ ∙ p · 𝟙 ∙ p · 𝟘) +ᶜ ((wk1Substₘ (wk1Substₘ Ψ)) *> γ) R₁.∎
  import Tools.Reasoning.PartialOrder ≤ᶜ-poset as R₂
  eq′ = R₂.begin
    Ψ *> ((r *) ·ᶜ (γ +ᶜ p ·ᶜ δ))      R₂.≈⟨ *>-distrib-·ᶜ Ψ _ _ ⟩
    (r *) ·ᶜ (Ψ *> (γ +ᶜ p ·ᶜ δ))      R₂.≈⟨ ·ᶜ-cong ≈-refl (*>-distrib-+ᶜ Ψ _ _) ⟩
    (r *) ·ᶜ (Ψ *> γ +ᶜ Ψ *> (p ·ᶜ δ)) R₂.≈⟨ ·ᶜ-cong ≈-refl (+ᶜ-cong ≈ᶜ-refl (*>-distrib-·ᶜ Ψ _ _)) ⟩
    (r *) ·ᶜ (Ψ *> γ +ᶜ p ·ᶜ Ψ *> δ)   R₂.∎
...
-- substₘ-calc-correct : {𝕄 : Modality M} {Γ : Con (Term M) m} {γ : Conₘ m} {A : Term M m}
...
-- substₘ-calc-upper-bound : {𝕄 : Modality M} {γ : Conₘ m} → (σ : Subst M m n) → (x : Fin n) → γ ▸ σ x → γ ≤ᶜ ∥ σ ∥ *> (𝟘ᶜ , x ≔ Modality.𝟙 𝕄)
...
-- wf-compSubst : ∀ {𝕄 : Modality M} {Ψ : Substₘ m ℓ} {Φ : Substₘ ℓ n} {σ : Subst M m ℓ} {σ′ : Subst M ℓ n}
--              → Ψ ▶ σ → Φ ▶ σ′ → (Ψ <*> Φ) ▶ (σ ₛ•ₛ σ′)
