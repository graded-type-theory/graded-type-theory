...
This module presents some aspects of the formalisation relevant to the
paper "Normalisation for First-Class Universe Levels", Nils Anders
Danielsson, Naïm Camille Favier and Ondřej Kubánek, Proceedings of the
ACM on Programming Languages, Volume 10, Issue POPL, 2026
...
```agda
module README.First-class-universe-levels where
```
...
The code also supports top-level definitions, and definition contexts
are used in addition to the variable contexts in the paper.
```agda
import Definition.Untyped
  using (Con)
```
...
notation `∇ » ρ ∷ʷ Δ ⊇ Γ` means that ρ is a well-formed weakening from
Γ to Δ under the definition context ∇ (in the paper the notation
...
that `Var-included` is instantiated to `No-equality-reflection`.
```agda
...
import Graded.Erasure.SucRed
  using (_⊢_⇒ˢ_; _⊢_⇒ˢ_∷ℕ; _⊢_⇒ˢ*_∷ℕ; _⊢_⇒ˢ⟨_⟩*_)
```
