...
Σʳ : ([Γ] : ⊩ᵛ Γ) → Γ ⊢ Σ q ▷ F ▹ G ∷ U
   → ∃ λ ([U] : Γ ⊩ᵛ⟨ ¹ ⟩ U / [Γ])
   → γ ▸ Γ ⊩ʳ⟨ ¹ ⟩ Σ⟨ s ⟩ q ▷ F ▹ G ∷[ m ] U / [Γ] / [U]
Σʳ {F = F} {G = G} [Γ] ⊢Σ =
    [U]
  , subsumptionMode (Σ⟨ _ ⟩ _ ▷ F ▹ G) [U]
...
prodʳ : ∀ {l}
      → ([Γ] : ⊩ᵛ Γ)
        ...
        ([t] : Γ ⊩ᵛ⟨ l ⟩ t ∷ F / [Γ] / [F])
        ([u] : Γ ⊩ᵛ⟨ l ⟩ u ∷ G [ t ] / [Γ] / [G[t]])
        (⊩ʳt : γ ▸ Γ ⊩ʳ⟨ l ⟩ t ∷[ m ] F / [Γ] / [F])
        (⊩ʳu : δ ▸ Γ ⊩ʳ⟨ l ⟩ u ∷[ m ] G [ t ] / [Γ] / [G[t]])
      → ∃ λ ([Σ] : Γ ⊩ᵛ⟨ l ⟩ Σ⟨ s ⟩ q ▷ F ▹ G / [Γ])
      → γ +ᶜ δ ▸ Γ ⊩ʳ⟨ l ⟩ prod s t u ∷[ m ] Σ q ▷ F ▹ G / [Γ] / [Σ]
prodʳ
  {Γ = Γ} {F = F} {G = G} {t = t} {u = u} {γ = γ} {m = m} {δ = δ}
  {s = s} {q = q} {l = l} [Γ] [F] [G] [G[t]] [t] [u] ⊩ʳt ⊩ʳu =
  [Σ] , lemma m ⊩ʳt ⊩ʳu
  where
  [Σ] = Σᵛ [Γ] [F] [G]
...
fstʳ′ : ∀ {l} {Γ : Con Term n}
      → ([Γ] : ⊩ᵛ Γ)
        ([F] : Γ ⊩ᵛ⟨ l ⟩ F / [Γ])
        ([G] : Γ ∙ F ⊩ᵛ⟨ l ⟩ G / [Γ] ∙ [F])
        ([t] : Γ ⊩ᵛ⟨ l ⟩ t ∷ Σₚ q ▷ F ▹ G / [Γ] / Σᵛ [Γ] [F] [G])
        (⊩ʳt : γ ▸ Γ ⊩ʳ⟨ l ⟩ t ∷[ m ] Σₚ q ▷ F ▹ G
               / [Γ] / Σᵛ [Γ] [F] [G])
      → γ ▸ Γ ⊩ʳ⟨ l ⟩ fst t ∷[ m ] F / [Γ] / [F]
fstʳ′ {m = 𝟘ᵐ} = _

fstʳ′ {F = F} {G = G} {t = t} {q = q} {m = 𝟙ᵐ}
      [Γ] [F] [G] [t] ⊩ʳt {σ = σ} [σ] σ®σ′ =
...
sndʳ′ : ∀ {l} {Γ : Con Term n}
      → ([Γ] : ⊩ᵛ Γ)
        ...
        (⊩ʳt : γ ▸ Γ ⊩ʳ⟨ l ⟩ t ∷[ m ] Σₚ q ▷ F ▹ G / [Γ] / Σᵛ {F = F} {G = G} {q = q} [Γ] [F] [G])
      → ∃ λ ([G] : Γ ⊩ᵛ⟨ l ⟩ G [ fst t ] / [Γ])
      → γ ▸ Γ ⊩ʳ⟨ l ⟩ snd t ∷[ m ] G [ fst t ] / [Γ] / [G]
sndʳ′ {F = F} {G = G} {t = t} {q = q} {m = m} {l = l} {Γ = Γ}
      [Γ] [F] [G] [t] ⊩ʳt =
  [G[t₁]] , lemma m ⊩ʳt
  where
  [Σ] = Σᵛ [Γ] [F] [G]
  [t₁] = fstᵛ {t = t} [Γ] [F] [G] [t]
  [G[t₁]] = substSΠ (BΣ Σₚ q) [Γ] [F] [Σ] [t₁]

  lemma :
    ∀ m
    (⊩ʳt : γ ▸ Γ ⊩ʳ⟨ l ⟩ t ∷[ m ] Σₚ q ▷ F ▹ G / [Γ] / Σᵛ [Γ] [F] [G]) →
    γ ▸ Γ ⊩ʳ⟨ l ⟩ snd t ∷[ m ] G [ fst t ] / [Γ] / [G[t₁]]
  lemma 𝟘ᵐ = _
...
sndʳ : Γ ⊢ F → Γ ∙ F ⊢ G → Γ ⊢ t ∷ Σₚ q ▷ F ▹ G
     ...
     → (⊩ʳt : γ ▸ Γ ⊩ʳ⟨ ¹ ⟩ t ∷[ m ] Σₚ q ▷ F ▹ G / [Γ] / [Σ])
     → ∃ λ ([G] : Γ ⊩ᵛ⟨ ¹ ⟩ G [ fst t ] / [Γ])
     → γ ▸ Γ ⊩ʳ⟨ ¹ ⟩ snd t ∷[ m ] G [ fst t ] / [Γ] / [G]
...
prodrecʳ′ : ∀ {l} {Γ : Con Term n}
         → ([Γ] : ⊩ᵛ Γ)
          ...
          → let [Σ] = Σᵛ {F = F} {G} {q = q} {m = Σᵣ} [Γ] [F] [G] in
           ...
           (⊩ʳt : γ ▸ Γ ⊩ʳ⟨ l ⟩ t ∷[ ⌞ p ⌟ ] Σᵣ q ▷ F ▹ G / [Γ] / [Σ])
           (⊩ʳu : δ ∙ p ∙ p ▸ Γ ∙ F ∙ G ⊩ʳ⟨ l ⟩ u
                  ∷[ 𝟙ᵐ ] A [ prodᵣ (var (x0 +1)) (var x0) ]↑²
                  / [Γ] ∙ [F] ∙ [G] / [A₊])
           ([At] : Γ ⊩ᵛ⟨ l ⟩ A [ t ] / [Γ])
           ([u] : Γ ∙ F ∙ G ⊩ᵛ⟨ l ⟩ u ∷ A [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F] ∙ [G] / [A₊])
           ([σ] : ε ⊩ˢ σ ∷ Γ / [Γ] / ε)
           (σ®σ′ : σ ®⟨ l ⟩ σ′ ∷[ 𝟙ᵐ ] Γ ◂ p ·ᶜ γ +ᶜ δ / [Γ] / [σ])
...
prodrecʳ′ {n = n} {F = F} {G = G} {q = q} {A = A} {γ = γ} {t = t}
          {p = 𝟘} {δ = δ} {u = u} {σ = σ} {σ′ = σ′} {l = l} {Γ = Γ}
...
prodrecʳ : ∀ {l} {Γ : Con Term n}
         → ([Γ] : ⊩ᵛ Γ)
           ...
           ([u] : Γ ∙ F ∙ G ⊩ᵛ⟨ l ⟩ u ∷ A [ prodᵣ (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F] ∙ [G] / [A₊])
           (⊩ʳt : γ ▸ Γ ⊩ʳ⟨ l ⟩ t ∷[ m ᵐ· p ] Σᵣ q ▷ F ▹ G / [Γ] / [Σ])
           (⊩ʳu : δ ∙ ⌜ m ⌝ · p ∙ ⌜ m ⌝ · p ▸ Γ ∙ F ∙ G ⊩ʳ⟨ l ⟩ u
                  ∷[ m ] A [ prodᵣ (var (x0 +1)) (var x0) ]↑²
                  / [Γ] ∙ [F] ∙ [G] / [A₊])
         → ∃ λ ([At] : Γ ⊩ᵛ⟨ l ⟩ A [ t ] / [Γ])
         → p ·ᶜ γ +ᶜ δ ▸ Γ ⊩ʳ⟨ l ⟩ prodrec p A t u
           ∷[ m ] A [ t ] / [Γ] / [At]
prodrecʳ
  {n = n} {F = F} {G = G} {q = q} {A = A} {t = t} {u = u} {γ = γ}
  {m = m} {p = p} {δ = δ} {l = l} {Γ = Γ}
  [Γ] [F] [G] [Σ] [A] [A₊] [t] [u] ⊩ʳt ⊩ʳu =
  [At] , lemma m ⊩ʳt ⊩ʳu
  where
  [At] = substS {F = Σ q ▷ F ▹ G} {A} {t} [Γ] [Σ] [A] [t]
  ...
  lemma :
    ...
    (⊩ʳu : δ ∙ ⌜ m ⌝ · p ∙ ⌜ m ⌝ · p ▸ Γ ∙ F ∙ G ⊩ʳ⟨ l ⟩ u
           ∷[ m ] A [ prodᵣ (var (x0 +1)) (var x0) ]↑²
           / [Γ] ∙ [F] ∙ [G] / [A₊]) →
    p ·ᶜ γ +ᶜ δ ▸ Γ ⊩ʳ⟨ l ⟩ prodrec p A t u ∷[ m ] A [ t ] / [Γ] / [At]
  lemma 𝟘ᵐ = _
