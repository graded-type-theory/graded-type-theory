...
fstʳ′ {F = F} {G} {t = t} {q = q} [Γ] [F] [G] [t] ⊩ʳt {σ = σ} [σ] σ®σ′ =
  let [σF] = proj₁ (unwrap [F] ε [σ])
      ...
      _ , _ , _ , _ , _ , _ , ⊢t₁ , ⊢t₂ , Σ≡Σ′ = inversion-prod ⊢t′
...
sndʳ′ {F = F} {G} {t} {q = q} [Γ] [F] [G] [t] ⊩ʳt =
  ...
  in  [G[t₁]] , λ {σ = σ} [σ] σ®σ′ →
      let t₁ , t₂ , v₁ , v₂ , t⇒t′ , v⇒v′ , [t₁] , t₁®v₁ , t₂®v₂ = ⊩ʳt [σ] σ®σ′
          ...
          ⊢σG = escape [σG]
          _ , _ , ⊢t′ = syntacticRedTerm t⇒t′
          _ , _ , _ , _ , _ , _ , ⊢t₁ , ⊢t₂ , eq = inversion-prod ⊢t′
...
prodrecʳ′ : ∀ {l} {Γ : Con Term n}
         → ([Γ] : ⊩ᵛ Γ)
          ...
          → let [Σ] = Σᵛ {F = F} {G} {q = q} {m = Σᵣ} [Γ] [F] [G] in
           ...
           (⊩ʳu : δ ∙ p ∙ p ▸ Γ ∙ F ∙ G ⊩ʳ⟨ l ⟩ u ∷ A [ prod (var (x0 +1)) (var x0) ]↑² / [Γ] ∙ [F] ∙ [G] / [A₊])
           ([At] : Γ ⊩ᵛ⟨ l ⟩ A [ t ] / [Γ])
...
prodrecʳ′ {n} {F} {G} {q} {A} {γ} {t} {δ} {𝟘} {u} {σ} {σ′} {l} {Γ}
          [Γ] [F] [G] [A] [A₊] ⊩ʳt ⊩ʳu [At] [u] [σ] σ®σ′ (Σₜ p d p≡p (prodₙ {t = p₁} {u = p₂}) (wk[p₁] , wk[p₂])) =
  ...
  in  convTermʳ [σ₊A₊] [σAt]
                ...
                (PE.subst (λ x → subst σ (prodrec 𝟘 A t u) ®⟨ l ⟩ x
                               ∷ subst (consSubst (consSubst σ p₁) p₂) (A [ prod (var (x0 +1)) (var x0) ]↑²)
                               / [σ₊A₊])
...
prodrecʳ′ {n} {F} {G} {q} {A} {γ} {t} {δ} {ω} {u} {σ} {σ′} {l} {Γ}
          [Γ] [F] [G] [A] [A₊] ⊩ʳt ⊩ʳu [At] [u] [σ] σ®σ′ (Σₜ p d p≡p prodₙ (wk[p₁]′ , wk[p₂]))
